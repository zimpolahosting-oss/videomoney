const extraLocalizedValues = {
  'pt': {
    'appName': 'VideoMoney',
    'startingApp': 'A iniciar o VideoMoney',
    'preparingStartup':
        'A preparar o Firebase, os dados da carteira e os anúncios recompensados...',
    'startupFailed': 'Falha ao iniciar',
    'loadingWallet': 'A carregar a sua carteira...',
    'unableRestoreSession':
        'Não foi possível restaurar a sua sessão agora.',
    'welcomeBack': 'Bem-vindo de volta',
    'createYourAccount': 'Crie a sua conta',
    'signInBody':
        'Inicie sessão para continuar a ganhar visualizações e gerir pagamentos.',
    'signUpBody':
        'Registe-se para começar a ver vídeos recompensados e aumentar o seu saldo.',
    'login': 'Iniciar sessão',
    'signUp': 'Registar-se',
    'email': 'E-mail',
    'password': 'Palavra-passe',
    'enterYourEmail': 'Introduza o seu e-mail.',
    'enterValidEmail': 'Introduza um endereço de e-mail válido.',
    'enterYourPassword': 'Introduza a sua palavra-passe.',
    'useAtLeast6Chars': 'Utilize pelo menos 6 caracteres.',
    'rememberMe': 'Lembrar-me',
    'forgotPassword': 'Esqueceu-se da palavra-passe?',
    'resetPassword': 'Repor palavra-passe',
    'emailHint': 'name@example.com',
    'cancel': 'Cancelar',
    'sendResetLink': 'Enviar link de reposição',
    'passwordResetEmailSent':
        'E-mail de reposição enviado para {email}.',
    'unableSendResetEmail':
        'Não foi possível enviar o e-mail de reposição.',
    'firebaseAuthNotice':
        'Firebase Authentication permanece inalterado. Este redesign apenas atualiza a camada de apresentação.',
    'createAccount': 'Criar conta',
    'secureAccessDashboard':
        'Acesso seguro ao seu painel de ganhos',
    'firestoreBalanceStored':
        'O seu histórico de pagamentos e saldo de recompensas são armazenados no Firestore',
    'authenticationFailed': 'A autenticação falhou.',
    'noUserSessionFound': 'Nenhuma sessão de utilizador encontrada.',
    'home': 'Início',
    'earn': 'Ganhar',
    'wallet': 'Carteira',
    'profile': 'Perfil',
    'rewardConfirmedViewsAdded':
        'Recompensa confirmada. Visualizações adicionadas.',
    'rewardedAdNotCompleted':
        'O anúncio recompensado não foi concluído.',
    'watchVideo': 'Ver vídeo',
    'loading': 'A carregar...',
    'welcomeBackShort': 'Bem-vindo de volta,',
    'signedInUser': 'Utilizador com sessão iniciada',
    'watchVideosEarnPaid':
        'Veja vídeos, ganhe visualizações e receba pagamentos.',
    'currentViews': 'Visualizações atuais',
    'videosWatched': 'Vídeos vistos',
    'progressToPayout': 'Progresso até ao pagamento',
    'youAreOnYourWay': 'Está no caminho certo.',
    'earnViewsNow': 'Ganhe visualizações agora.',
    'payoutUnlocked':
        'Pagamento desbloqueado. Pode solicitar um pagamento na Carteira.',
    'moreViewsUntilPayout':
        'Faltam {views} visualizações para o pagamento.',
    'dailyBonus': 'Bónus diário',
    'watchDailyVideosBonus':
        'Veja {videos} vídeos por dia para obter visualizações de bónus.',
    'bonusClaimed': 'Bónus reclamado',
    'bonus': 'Bónus',
    'earnViewsTitle': 'Ganhar visualizações',
    'watchRewardedEarnViews':
        'Veja vídeos recompensados e ganhe visualizações instantaneamente.',
    'earnViews': 'Ganhar visualizações',
    'howItWorks': 'Como funciona',
    'watch': 'Ver',
    'watchShortVideo': 'Ver um vídeo curto',
    'earnStep': 'Ganhar',
    'getViewsReward': 'Receba visualizações como recompensa',
    'cashOut': 'Levantar',
    'reachViews': 'Atinga {views} visualizações',
    'dailyChallenge': 'Desafio diário',
    'watchTodayVideosBonus':
        'Veja {videos} vídeos hoje e receba visualizações de bónus!',
    'totalVideosWatched': 'Total de vídeos vistos',
    'memberSince': 'Membro desde: {date}',
    'notAvailableYet': 'Ainda não disponível',
    'emailVerified': 'E-mail verificado',
    'emailNotVerified': 'E-mail não verificado',
    'security': 'Segurança',
    'firebaseProtected': 'Protegido pelo Firebase',
    'payoutReview': 'Revisão de pagamento',
    'adminApproval': 'Aprovação do administrador',
    'readAdminReplies': 'Ler respostas e notificações do administrador',
    'notificationsPrivacy':
        'Notificações, privacidade e definições da app',
    'contactAdmin': 'Contactar o administrador e enviar mensagens',
    'reportProblemBug': 'Reportar um problema ou erro',
    'rateApp': 'Avaliar app',
    'sendInAppRating':
        'Envie a sua avaliação na app de 1 a 5 estrelas',
    'aboutVideoMoney': 'Sobre o VideoMoney',
    'reviewPayoutRequests': 'Rever pedidos de pagamento',
    'logout': 'Terminar sessão',
    'yourWallet': 'A sua carteira',
    'availableViews': 'Visualizações disponíveis',
    'estimatedPayout': 'Pagamento estimado',
    'remainingToPayout': 'Restante até ao pagamento',
    'viewsUnit': 'visualizações',
    'estimateOnly':
        'Apenas uma estimativa. 50 visualizações ≈ €0,01 e os ganhos reais podem variar.',
    'requestPayout': 'Solicitar pagamento',
    'minPayout': 'Pagamento mín.',
    'processingTime': 'Tempo de processamento',
    'approval': 'Aprovação',
    'adminReview': 'Revisão do administrador',
    'payoutMethods': 'Métodos de pagamento',
    'payoutHistory': 'Histórico de pagamentos',
    'noPayoutRequestsYet': 'Ainda não existem pedidos de pagamento.',
    'pendingTimestamp': 'Data pendente',
    'paypalSubtitle': 'Solicite pagamento em EUR, GBP ou USD',
    'revolutSubtitle':
        'Pagamento rápido para a carteira na moeda escolhida',
    'bankTransferTitle': 'Transferência bancária',
    'bankTransferSubtitle':
        'Adicione IBAN ou número de conta bancária para pagamento manual',
    'viewFullHistory': 'Ver histórico completo',
    'inbox': 'Caixa de entrada',
    'markAllRead': 'Marcar tudo como lido',
    'noInboxMessagesYet':
        'Ainda não existem mensagens na caixa de entrada.',
    'newBadge': 'NOVO',
    'rateAppQuestion': 'Como avaliaria o VideoMoney?',
    'choose1to5':
        'Escolha entre 1 e 5 estrelas. Pode atualizar a sua avaliação mais tarde.',
    'saveRating': 'Guardar avaliação',
    'thanksForRating': 'Obrigado pela sua avaliação.',
    'requestPayoutTitle': 'Solicitar pagamento',
    'payoutRules': 'Regras de pagamento',
    'minimumPayoutIs':
        'O pagamento mínimo é de {views} visualizações.',
    'processingCanTake':
        'O processamento pode demorar até {days} dias após a aprovação do administrador.',
    'everyRequestReviewed':
        'Cada pedido é revisto pelo administrador antes de ser pago.',
    'useBankAddIban':
        'Utilize Banco para transferência manual e adicione o seu IBAN ou número de conta bancária.',
    'submitUsingBalance':
        'Envie um pedido de pagamento utilizando o seu saldo de visualizações.',
    'estimatedEarningsNotGuaranteed':
        'Apenas ganhos estimados. 50 visualizações concluídas ≈ €0,01 e isto não é uma promessa de pagamento garantida.',
    'payoutCurrency': 'Moeda de pagamento',
    'payoutMethod': 'Método de pagamento',
    'viewsToRequest': 'Visualizações a solicitar',
    'minimumViewsHelper': 'Mínimo de 10.000 visualizações',
    'enterAmount': 'Introduza um montante.',
    'enterValidPositiveNumber':
        'Introduza um número positivo válido.',
    'accountHolderName': 'Nome do titular da conta',
    'enterAccountHolderName':
        'Introduza o nome do titular da conta.',
    'paypalEmail': 'E-mail do PayPal',
    'enterPaypalEmail': 'Introduza um e-mail do PayPal.',
    'enterValidPaypalEmail':
        'Introduza um e-mail do PayPal válido.',
    'revolutUsername': 'Nome de utilizador do Revolut',
    'revolutExample': 'Exemplo: @yourname',
    'enterRevolutUsername':
        'Introduza o seu nome de utilizador do Revolut.',
    'bankName': 'Nome do banco',
    'enterBankName': 'Introduza o nome do seu banco.',
    'iban': 'IBAN',
    'ibanOptional':
        'Opcional se fornecer um número de conta bancária',
    'bankAccountNumber': 'Número de conta bancária',
    'bankRequiredIfNoIban':
        'Obrigatório se não introduzir um IBAN',
    'enterIbanOrBank':
        'Introduza um IBAN ou número de conta bancária.',
    'submitRequest': 'Enviar pedido',
    'payoutRequestSubmitted': 'Pedido de pagamento enviado.',
    'payoutHistoryTitle': 'Histórico de pagamentos',
    'unableLoadPayoutHistory':
        'Não foi possível carregar o histórico de pagamentos agora.',
    'currencyLabel': 'Moeda: {currency}',
    'accountHolderLabel': 'Titular da conta: {value}',
    'notProvided': 'Não fornecido',
    'helpSupport': 'Ajuda e suporte',
    'openSupportTicket': 'Abrir um ticket de suporte',
    'describeIssueAdminReply':
        'Descreva o seu problema e o administrador pode responder na sua caixa de entrada.',
    'subject': 'Assunto',
    'helpSubjectHint': 'Em que precisa de ajuda?',
    'message': 'Mensagem',
    'messageHint': 'Escreva a sua mensagem aqui...',
    'send': 'Enviar',
    'sending': 'A enviar...',
    'openInbox': 'Abrir caixa de entrada',
    'yourTickets': 'Os seus tickets',
    'noSupportTicketsYet': 'Ainda não existem tickets de suporte.',
    'supportMessageSent': 'Mensagem de suporte enviada.',
    'adminReply': 'Resposta do administrador: {value}',
    'reportBug': 'Reportar erro',
    'tellUsWhatHappened': 'Conte-nos o que aconteceu',
    'includeStepsExpected':
        'Inclua os passos para reproduzir e o que esperava ver.',
    'title': 'Título',
    'shortSummary': 'Resumo curto',
    'description': 'Descrição',
    'describeBug': 'Descreva o erro...',
    'submit': 'Enviar',
    'bugReportSubmitted': 'Relatório de erro enviado.',
    'aboutTagline': 'Tema escuro premium • Interface verde néon',
    'version': 'Versão',
    'minimumPayoutLabel': 'Pagamento mínimo',
    'processingTimeLabel': 'Tempo de processamento',
    'reviewLabel': 'Revisão',
    'estimatedEarningsOnlyPolicies':
        'Apenas ganhos estimados. Os ganhos reais podem variar com base no desempenho dos anúncios e nas regras de política.',
    'privacyPolicy': 'Política de privacidade',
    'termsOfService': 'Termos de serviço',
    'notifications': 'Notificações',
    'enableNotifications': 'Ativar notificações',
    'generalAppNotifications': 'Notificações gerais da app',
    'dailyReminder': 'Lembrete diário',
    'dailyReminderSubtitle':
        'Receba um lembrete para concluir o bónus diário',
    'settingsSaved': 'Definições guardadas.',
    'privacy': 'Privacidade',
    'appVersion': 'Versão da app',
    'save': 'Guardar',
    'saving': 'A guardar...',
    'termsUsingTitle': 'Utilizar o VideoMoney',
    'termsViewsTitle': 'Visualizações e recompensas',
    'termsPayoutsTitle': 'Pagamentos e revisão',
    'termsSupportTitle': 'Suporte e mensagens',
    'termsUsingBullet1':
        'Deve utilizar informações de conta corretas.',
    'termsUsingBullet2':
        'Uma pessoa não pode abusar de múltiplas contas, bots, scripts, rotação de VPN ou quintas de emuladores para gerar visualizações extra.',
    'termsUsingBullet3':
        'Os anúncios recompensados, a autenticação Firebase e a revisão de pagamentos permanecem protegidos pela configuração atual da plataforma.',
    'termsViewsBullet1':
        'As visualizações mostradas na app são unidades promocionais de recompensa usadas dentro do VideoMoney.',
    'termsViewsBullet2':
        'Os ganhos estimados são apenas informativos e podem mudar com base no desempenho da plataforma, na política, nas verificações de fraude e na revisão de pagamentos.',
    'termsViewsBullet3':
        'As recompensas de bónus diário são limitadas à atividade elegível e podem ser removidas se for detetado abuso.',
    'termsPayoutsBullet1':
        'O pagamento mínimo continua a ser 10.000 visualizações.',
    'termsPayoutsBullet2':
        'Todos os pedidos de pagamento exigem aprovação manual do administrador e podem ser aprovados, rejeitados ou marcados como pagos.',
    'termsPayoutsBullet3':
        'Os pedidos de pagamento rejeitados podem ser reembolsados ao saldo do utilizador quando permitido pelo fluxo de trabalho do administrador.',
    'termsSupportBullet1':
        'Ajuda e suporte, relatórios de erro, respostas do administrador e notificações push podem ser armazenados na sua caixa de entrada na app.',
    'termsSupportBullet2':
        'Ao ativar as notificações, permite que o VideoMoney envie atualizações da app, respostas de suporte e lembretes diários para o seu dispositivo.',
    'termsSupportBullet3':
        'Uso indevido grave, assédio ou atividade fraudulenta podem levar à restrição do acesso à app.',
    'settingsTitle': 'Definições',
    'statusPending': 'PENDENTE',
    'statusApproved': 'APROVADO',
    'statusPaid': 'PAGO',
    'statusRejected': 'REJEITADO',
    'statusProcessing': 'EM PROCESSAMENTO',
    'statusFixed': 'CORRIGIDO',
    'statusClosed': 'FECHADO',
    'typeSupport': 'SUPORTE',
    'typePayment': 'PAGAMENTO',
    'typeBug': 'ERRO',
  },
  'it': {
    'appName': 'VideoMoney',
    'startingApp': 'Avvio di VideoMoney',
    'preparingStartup':
        'Preparazione di Firebase, dati del portafoglio e annunci con premio...',
    'startupFailed': 'Avvio non riuscito',
    'loadingWallet': 'Caricamento del tuo portafoglio...',
    'unableRestoreSession':
        'Impossibile ripristinare la tua sessione in questo momento.',
    'welcomeBack': 'Bentornato',
    'createYourAccount': 'Crea il tuo account',
    'signInBody':
        'Accedi per continuare a guadagnare visualizzazioni e gestire i pagamenti.',
    'signUpBody':
        'Registrati per iniziare a guardare video con premio e aumentare il tuo saldo.',
    'login': 'Accedi',
    'signUp': 'Registrati',
    'email': 'E-mail',
    'password': 'Password',
    'enterYourEmail': 'Inserisci la tua e-mail.',
    'enterValidEmail': 'Inserisci un indirizzo e-mail valido.',
    'enterYourPassword': 'Inserisci la tua password.',
    'useAtLeast6Chars': 'Usa almeno 6 caratteri.',
    'rememberMe': 'Ricordami',
    'forgotPassword': 'Password dimenticata?',
    'resetPassword': 'Reimposta password',
    'emailHint': 'name@example.com',
    'cancel': 'Annulla',
    'sendResetLink': 'Invia link di reimpostazione',
    'passwordResetEmailSent':
        'E-mail di reimpostazione password inviata a {email}.',
    'unableSendResetEmail':
        'Impossibile inviare l e-mail di reimpostazione.',
    'firebaseAuthNotice':
        'Firebase Authentication resta invariato. Questo redesign aggiorna solo il livello di presentazione.',
    'createAccount': 'Crea account',
    'secureAccessDashboard':
        'Accesso sicuro alla tua dashboard dei guadagni',
    'firestoreBalanceStored':
        'La cronologia dei pagamenti e il saldo delle ricompense sono archiviati in Firestore',
    'authenticationFailed': 'Autenticazione non riuscita.',
    'noUserSessionFound': 'Nessuna sessione utente trovata.',
    'home': 'Home',
    'earn': 'Guadagna',
    'wallet': 'Portafoglio',
    'profile': 'Profilo',
    'rewardConfirmedViewsAdded':
        'Ricompensa confermata. Visualizzazioni aggiunte.',
    'rewardedAdNotCompleted':
        'Il video pubblicitario con premio non è stato completato.',
    'watchVideo': 'Guarda video',
    'loading': 'Caricamento...',
    'welcomeBackShort': 'Bentornato,',
    'signedInUser': 'Utente con accesso effettuato',
    'watchVideosEarnPaid':
        'Guarda video, guadagna visualizzazioni e ricevi pagamenti.',
    'currentViews': 'Visualizzazioni attuali',
    'videosWatched': 'Video guardati',
    'progressToPayout': 'Avanzamento verso il pagamento',
    'youAreOnYourWay': 'Sei sulla buona strada.',
    'earnViewsNow': 'Guadagna visualizzazioni ora.',
    'payoutUnlocked':
        'Pagamento sbloccato. Puoi richiedere un pagamento nel Portafoglio.',
    'moreViewsUntilPayout':
        'Mancano {views} visualizzazioni al pagamento.',
    'dailyBonus': 'Bonus giornaliero',
    'watchDailyVideosBonus':
        'Guarda {videos} video al giorno per ottenere visualizzazioni bonus.',
    'bonusClaimed': 'Bonus richiesto',
    'bonus': 'Bonus',
    'earnViewsTitle': 'Guadagna visualizzazioni',
    'watchRewardedEarnViews':
        'Guarda video con premio e guadagna visualizzazioni subito.',
    'earnViews': 'Guadagna visualizzazioni',
    'howItWorks': 'Come funziona',
    'watch': 'Guarda',
    'watchShortVideo': 'Guarda un breve video',
    'earnStep': 'Guadagna',
    'getViewsReward': 'Ottieni visualizzazioni come ricompensa',
    'cashOut': 'Incassa',
    'reachViews': 'Raggiungi {views} visualizzazioni',
    'dailyChallenge': 'Sfida giornaliera',
    'watchTodayVideosBonus':
        'Guarda {videos} video oggi e ottieni visualizzazioni bonus!',
    'totalVideosWatched': 'Totale video guardati',
    'memberSince': 'Membro dal: {date}',
    'notAvailableYet': 'Non ancora disponibile',
    'emailVerified': 'E-mail verificata',
    'emailNotVerified': 'E-mail non verificata',
    'security': 'Sicurezza',
    'firebaseProtected': 'Protetto da Firebase',
    'payoutReview': 'Revisione del pagamento',
    'adminApproval': 'Approvazione amministratore',
    'readAdminReplies': 'Leggi le risposte e le notifiche admin',
    'notificationsPrivacy':
        'Notifiche, privacy e impostazioni app',
    'contactAdmin': 'Contatta admin e invia messaggi',
    'reportProblemBug': 'Segnala un problema o un bug',
    'rateApp': 'Valuta app',
    'sendInAppRating':
        'Invia la tua valutazione in app da 1 a 5 stelle',
    'aboutVideoMoney': 'Informazioni su VideoMoney',
    'reviewPayoutRequests': 'Rivedi le richieste di pagamento',
    'logout': 'Esci',
    'yourWallet': 'Il tuo portafoglio',
    'availableViews': 'Visualizzazioni disponibili',
    'estimatedPayout': 'Pagamento stimato',
    'remainingToPayout': 'Mancante al pagamento',
    'viewsUnit': 'visualizzazioni',
    'estimateOnly':
        'Solo una stima. 50 visualizzazioni ≈ €0,01 e i guadagni effettivi possono variare.',
    'requestPayout': 'Richiedi pagamento',
    'minPayout': 'Pagamento min.',
    'processingTime': 'Tempo di elaborazione',
    'approval': 'Approvazione',
    'adminReview': 'Revisione admin',
    'payoutMethods': 'Metodi di pagamento',
    'payoutHistory': 'Cronologia pagamenti',
    'noPayoutRequestsYet': 'Ancora nessuna richiesta di pagamento.',
    'pendingTimestamp': 'Timestamp in sospeso',
    'paypalSubtitle':
        'Richiedi il pagamento in EUR, GBP o USD',
    'revolutSubtitle':
        'Pagamento rapido sul portafoglio con la valuta scelta',
    'bankTransferTitle': 'Bonifico bancario',
    'bankTransferSubtitle':
        'Aggiungi IBAN o numero di conto bancario per il pagamento manuale',
    'viewFullHistory': 'Vedi cronologia completa',
    'inbox': 'Posta in arrivo',
    'markAllRead': 'Segna tutto come letto',
    'noInboxMessagesYet':
        'Ancora nessun messaggio nella posta in arrivo.',
    'newBadge': 'NUOVO',
    'rateAppQuestion': 'Come valuteresti VideoMoney?',
    'choose1to5':
        'Scegli tra 1 e 5 stelle. Puoi aggiornare la tua valutazione più tardi.',
    'saveRating': 'Salva valutazione',
    'thanksForRating': 'Grazie per la tua valutazione.',
    'requestPayoutTitle': 'Richiedi pagamento',
    'payoutRules': 'Regole di pagamento',
    'minimumPayoutIs':
        'Il pagamento minimo è di {views} visualizzazioni.',
    'processingCanTake':
        'La procedura può richiedere fino a {days} giorni dopo l approvazione admin.',
    'everyRequestReviewed':
        'Ogni richiesta viene esaminata da admin prima del pagamento.',
    'useBankAddIban':
        'Usa Banca per il trasferimento manuale e aggiungi il tuo IBAN o numero di conto bancario.',
    'submitUsingBalance':
        'Invia una richiesta di pagamento usando il tuo saldo di visualizzazioni.',
    'estimatedEarningsNotGuaranteed':
        'Solo guadagni stimati. 50 visualizzazioni completate ≈ €0,01 e questa non è una promessa di pagamento garantita.',
    'payoutCurrency': 'Valuta di pagamento',
    'payoutMethod': 'Metodo di pagamento',
    'viewsToRequest': 'Visualizzazioni da richiedere',
    'minimumViewsHelper': 'Minimo 10.000 visualizzazioni',
    'enterAmount': 'Inserisci un importo.',
    'enterValidPositiveNumber':
        'Inserisci un numero positivo valido.',
    'accountHolderName': 'Nome del titolare del conto',
    'enterAccountHolderName':
        'Inserisci il nome del titolare del conto.',
    'paypalEmail': 'E-mail PayPal',
    'enterPaypalEmail': 'Inserisci una e-mail PayPal.',
    'enterValidPaypalEmail':
        'Inserisci una e-mail PayPal valida.',
    'revolutUsername': 'Nome utente Revolut',
    'revolutExample': 'Esempio: @yourname',
    'enterRevolutUsername':
        'Inserisci il tuo nome utente Revolut.',
    'bankName': 'Nome della banca',
    'enterBankName': 'Inserisci il nome della tua banca.',
    'iban': 'IBAN',
    'ibanOptional':
        'Facoltativo se fornisci un numero di conto bancario',
    'bankAccountNumber': 'Numero di conto bancario',
    'bankRequiredIfNoIban':
        'Obbligatorio se non inserisci un IBAN',
    'enterIbanOrBank':
        'Inserisci un IBAN o un numero di conto bancario.',
    'submitRequest': 'Invia richiesta',
    'payoutRequestSubmitted': 'Richiesta di pagamento inviata.',
    'payoutHistoryTitle': 'Cronologia pagamenti',
    'unableLoadPayoutHistory':
        'Impossibile caricare la cronologia pagamenti in questo momento.',
    'currencyLabel': 'Valuta: {currency}',
    'accountHolderLabel': 'Titolare del conto: {value}',
    'notProvided': 'Non fornito',
    'helpSupport': 'Aiuto e supporto',
    'openSupportTicket': 'Apri un ticket di supporto',
    'describeIssueAdminReply':
        'Descrivi il tuo problema e admin può rispondere nella tua posta in arrivo.',
    'subject': 'Oggetto',
    'helpSubjectHint': 'Per cosa ti serve aiuto?',
    'message': 'Messaggio',
    'messageHint': 'Scrivi qui il tuo messaggio...',
    'send': 'Invia',
    'sending': 'Invio in corso...',
    'openInbox': 'Apri posta in arrivo',
    'yourTickets': 'I tuoi ticket',
    'noSupportTicketsYet':
        'Ancora nessun ticket di supporto.',
    'supportMessageSent': 'Messaggio di supporto inviato.',
    'adminReply': 'Risposta admin: {value}',
    'reportBug': 'Segnala bug',
    'tellUsWhatHappened': 'Dicci cosa è successo',
    'includeStepsExpected':
        'Includi i passaggi per riprodurre il problema e ciò che ti aspettavi di vedere.',
    'title': 'Titolo',
    'shortSummary': 'Breve riepilogo',
    'description': 'Descrizione',
    'describeBug': 'Descrivi il bug...',
    'submit': 'Invia',
    'bugReportSubmitted': 'Segnalazione bug inviata.',
    'aboutTagline':
        'Tema scuro premium • Interfaccia verde neon',
    'version': 'Versione',
    'minimumPayoutLabel': 'Pagamento minimo',
    'processingTimeLabel': 'Tempo di elaborazione',
    'reviewLabel': 'Revisione',
    'estimatedEarningsOnlyPolicies':
        'Solo guadagni stimati. I guadagni reali possono variare in base al rendimento degli annunci e alle regole di policy.',
    'privacyPolicy': 'Informativa sulla privacy',
    'termsOfService': 'Termini di servizio',
    'notifications': 'Notifiche',
    'enableNotifications': 'Abilita notifiche',
    'generalAppNotifications': 'Notifiche generali app',
    'dailyReminder': 'Promemoria giornaliero',
    'dailyReminderSubtitle':
        'Ricevi un promemoria per completare il bonus giornaliero',
    'settingsSaved': 'Impostazioni salvate.',
    'privacy': 'Privacy',
    'appVersion': 'Versione app',
    'save': 'Salva',
    'saving': 'Salvataggio...',
    'termsUsingTitle': 'Usare VideoMoney',
    'termsViewsTitle': 'Visualizzazioni e ricompense',
    'termsPayoutsTitle': 'Pagamenti e revisione',
    'termsSupportTitle': 'Supporto e messaggi',
    'termsUsingBullet1':
        'Devi usare informazioni account accurate.',
    'termsUsingBullet2':
        'Una persona non può abusare di account multipli, bot, script, rotazione VPN o farm di emulatori per generare visualizzazioni extra.',
    'termsUsingBullet3':
        'Gli annunci con premio, Firebase Authentication e la revisione dei pagamenti restano protetti dalla configurazione esistente della piattaforma.',
    'termsViewsBullet1':
        'Le visualizzazioni mostrate in app sono unità promozionali di ricompensa usate dentro VideoMoney.',
    'termsViewsBullet2':
        'I guadagni stimati sono solo informativi e possono cambiare in base al rendimento della piattaforma, alla policy, ai controlli antifrode e alla revisione dei pagamenti.',
    'termsViewsBullet3':
        'Le ricompense bonus giornaliere sono limitate alle attività idonee e possono essere rimosse se viene rilevato abuso.',
    'termsPayoutsBullet1':
        'Il pagamento minimo resta 10.000 visualizzazioni.',
    'termsPayoutsBullet2':
        'Tutte le richieste di pagamento richiedono approvazione manuale admin e possono essere approvate, rifiutate o contrassegnate come pagate.',
    'termsPayoutsBullet3':
        'Le richieste di pagamento rifiutate possono essere rimborsate al saldo utente quando il flusso di lavoro admin lo consente.',
    'termsSupportBullet1':
        'Aiuto e supporto, segnalazioni bug, risposte admin e notifiche push possono essere archiviati nella tua posta in arrivo in app.',
    'termsSupportBullet2':
        'Attivando le notifiche, consenti a VideoMoney di inviare al tuo dispositivo aggiornamenti app, risposte di supporto e messaggi di promemoria giornalieri.',
    'termsSupportBullet3':
        'Uso improprio grave, molestie o attività fraudolente possono portare alla limitazione di accesso app.',
    'settingsTitle': 'Impostazioni',
    'statusPending': 'IN ATTESA',
    'statusApproved': 'APPROVATO',
    'statusPaid': 'PAGATO',
    'statusRejected': 'RIFIUTATO',
    'statusProcessing': 'IN ELABORAZIONE',
    'statusFixed': 'RISOLTO',
    'statusClosed': 'CHIUSO',
    'typeSupport': 'SUPPORTO',
    'typePayment': 'PAGAMENTO',
    'typeBug': 'BUG',
  },
  'tr': {
    'appName': 'VideoMoney',
    'startingApp': 'VideoMoney başlatılıyor',
    'preparingStartup':
        'Firebase, cüzdan verileri ve ödüllü reklamlar hazırlanıyor...',
    'startupFailed': 'Başlatma başarısız oldu',
    'loadingWallet': 'Cüzdanınız yükleniyor...',
    'unableRestoreSession':
        'Oturumunuz şu anda geri yüklenemiyor.',
    'welcomeBack': 'Tekrar hoş geldiniz',
    'createYourAccount': 'Hesabınızı oluşturun',
    'signInBody':
        'Görüntüleme kazanmaya devam etmek ve ödemeleri yönetmek için giriş yapın.',
    'signUpBody':
        'Ödüllü videolar izlemeye ve bakiyenizi oluşturmaya başlamak için kaydolun.',
    'login': 'Giriş yap',
    'signUp': 'Kaydol',
    'email': 'E-posta',
    'password': 'Şifre',
    'enterYourEmail': 'E-postanızı girin.',
    'enterValidEmail': 'Geçerli bir e-posta adresi girin.',
    'enterYourPassword': 'Şifrenizi girin.',
    'useAtLeast6Chars': 'En az 6 karakter kullanın.',
    'rememberMe': 'Beni hatırla',
    'forgotPassword': 'Şifrenizi mi unuttunuz?',
    'resetPassword': 'Şifreyi sıfırla',
    'emailHint': 'name@example.com',
    'cancel': 'İptal',
    'sendResetLink': 'Sıfırlama bağlantısı gönder',
    'passwordResetEmailSent':
        'Şifre sıfırlama e-postası {email} adresine gönderildi.',
    'unableSendResetEmail':
        'Şifre sıfırlama e-postası gönderilemedi.',
    'firebaseAuthNotice':
        'Firebase Authentication değişmeden kalır. Bu yeniden tasarım yalnızca sunum katmanını günceller.',
    'createAccount': 'Hesap oluştur',
    'secureAccessDashboard':
        'Kazanç panelinize güvenli erişim',
    'firestoreBalanceStored':
        'Ödeme geçmişiniz ve ödül bakiyeniz Firestore içinde saklanır',
    'authenticationFailed': 'Kimlik doğrulama başarısız oldu.',
    'noUserSessionFound': 'Kullanıcı oturumu bulunamadı.',
    'home': 'Ana sayfa',
    'earn': 'Kazan',
    'wallet': 'Cüzdan',
    'profile': 'Profil',
    'rewardConfirmedViewsAdded':
        'Ödül onaylandı. Görüntülemeler eklendi.',
    'rewardedAdNotCompleted':
        'Ödüllü reklam tamamlanmadı.',
    'watchVideo': 'Video izle',
    'loading': 'Yükleniyor...',
    'welcomeBackShort': 'Tekrar hoş geldiniz,',
    'signedInUser': 'Oturum açmış kullanıcı',
    'watchVideosEarnPaid':
        'Videolar izleyin, görüntüleme kazanın ve ödeme alın.',
    'currentViews': 'Mevcut görüntülemeler',
    'videosWatched': 'İzlenen videolar',
    'progressToPayout': 'Ödemeye ilerleme',
    'youAreOnYourWay': 'Doğru yoldasınız.',
    'earnViewsNow': 'Şimdi görüntüleme kazanın.',
    'payoutUnlocked':
        'Ödeme açıldı. Cüzdan bölümünden ödeme isteğinde bulunabilirsiniz.',
    'moreViewsUntilPayout':
        'Ödeme için {views} görüntüleme daha gerekli.',
    'dailyBonus': 'Günlük bonus',
    'watchDailyVideosBonus':
        'Bonus görüntüleme almak için her gün {videos} video izleyin.',
    'bonusClaimed': 'Bonus alındı',
    'bonus': 'Bonus',
    'earnViewsTitle': 'Görüntüleme kazan',
    'watchRewardedEarnViews':
        'Ödüllü videolar izleyin ve anında görüntüleme kazanın.',
    'earnViews': 'Görüntüleme kazan',
    'howItWorks': 'Nasıl çalışır',
    'watch': 'İzle',
    'watchShortVideo': 'Kısa bir video izle',
    'earnStep': 'Kazan',
    'getViewsReward': 'Ödül olarak görüntüleme alın',
    'cashOut': 'Nakit çek',
    'reachViews': '{views} görüntülemeye ulaşın',
    'dailyChallenge': 'Günlük görev',
    'watchTodayVideosBonus':
        'Bugün {videos} video izleyin ve bonus görüntüleme alın!',
    'totalVideosWatched': 'Toplam izlenen video',
    'memberSince': 'Üyelik tarihi: {date}',
    'notAvailableYet': 'Henüz mevcut değil',
    'emailVerified': 'E-posta doğrulandı',
    'emailNotVerified': 'E-posta doğrulanmadı',
    'security': 'Güvenlik',
    'firebaseProtected': 'Firebase ile korunur',
    'payoutReview': 'Ödeme incelemesi',
    'adminApproval': 'Yönetici onayı',
    'readAdminReplies':
        'Yönetici yanıtlarını ve bildirimleri okuyun',
    'notificationsPrivacy':
        'Bildirimler, gizlilik ve uygulama ayarları',
    'contactAdmin': 'Yöneticiyle iletişime geçin ve mesaj gönderin',
    'reportProblemBug': 'Bir sorun veya hata bildirin',
    'rateApp': 'Uygulamayı değerlendir',
    'sendInAppRating':
        'Uygulama içi puanınızı 1 ile 5 yıldız arasında gönderin',
    'aboutVideoMoney': 'VideoMoney hakkında',
    'reviewPayoutRequests': 'Ödeme isteklerini inceleyin',
    'logout': 'Çıkış yap',
    'yourWallet': 'Cüzdanınız',
    'availableViews': 'Kullanılabilir görüntülemeler',
    'estimatedPayout': 'Tahmini ödeme',
    'remainingToPayout': 'Ödemeye kalan',
    'viewsUnit': 'görüntüleme',
    'estimateOnly':
        'Yalnızca tahmindir. 50 görüntüleme ≈ €0.01 ve gerçek kazançlar değişebilir.',
    'requestPayout': 'Ödeme iste',
    'minPayout': 'Min. ödeme',
    'processingTime': 'İşlem süresi',
    'approval': 'Onay',
    'adminReview': 'Yönetici incelemesi',
    'payoutMethods': 'Ödeme yöntemleri',
    'payoutHistory': 'Ödeme geçmişi',
    'noPayoutRequestsYet': 'Henüz ödeme isteği yok.',
    'pendingTimestamp': 'Bekleyen zaman damgası',
    'paypalSubtitle':
        'EUR, GBP veya USD cinsinden ödeme isteyin',
    'revolutSubtitle':
        'Seçilen para birimiyle hızlı cüzdan ödemesi',
    'bankTransferTitle': 'Banka havalesi',
    'bankTransferSubtitle':
        'Manuel ödeme için IBAN veya banka hesap numarası ekleyin',
    'viewFullHistory': 'Tüm geçmişi görüntüle',
    'inbox': 'Gelen kutusu',
    'markAllRead': 'Tümünü okundu olarak işaretle',
    'noInboxMessagesYet': 'Henüz gelen kutusu mesajı yok.',
    'newBadge': 'YENİ',
    'rateAppQuestion':
        'VideoMoney uygulamasını nasıl değerlendirirsiniz?',
    'choose1to5':
        '1 ile 5 yıldız arasında seçin. Puanınızı daha sonra güncelleyebilirsiniz.',
    'saveRating': 'Puanı kaydet',
    'thanksForRating': 'Puanınız için teşekkürler.',
    'requestPayoutTitle': 'Ödeme iste',
    'payoutRules': 'Ödeme kuralları',
    'minimumPayoutIs': 'Minimum ödeme {views} görüntülemedir.',
    'processingCanTake':
        'İşlem, yönetici onayından sonra en fazla {days} gün sürebilir.',
    'everyRequestReviewed':
        'Her istek ödenmeden önce yönetici tarafından incelenir.',
    'useBankAddIban':
        'Manuel transfer için Banka seçeneğini kullanın ve IBAN veya banka hesap numaranızı ekleyin.',
    'submitUsingBalance':
        'Görüntüleme bakiyenizi kullanarak bir ödeme isteği gönderin.',
    'estimatedEarningsNotGuaranteed':
        'Yalnızca tahmini kazançlar. 50 tamamlanmış görüntüleme ≈ €0.01 ve bu garantili bir ödeme vaadi değildir.',
    'payoutCurrency': 'Ödeme para birimi',
    'payoutMethod': 'Ödeme yöntemi',
    'viewsToRequest': 'İstenecek görüntülemeler',
    'minimumViewsHelper': 'Minimum 10.000 görüntüleme',
    'enterAmount': 'Bir tutar girin.',
    'enterValidPositiveNumber':
        'Geçerli bir pozitif sayı girin.',
    'accountHolderName': 'Hesap sahibi adı',
    'enterAccountHolderName':
        'Hesap sahibinin adını girin.',
    'paypalEmail': 'PayPal e-postası',
    'enterPaypalEmail': 'Bir PayPal e-postası girin.',
    'enterValidPaypalEmail':
        'Geçerli bir PayPal e-postası girin.',
    'revolutUsername': 'Revolut kullanıcı adı',
    'revolutExample': 'Örnek: @yourname',
    'enterRevolutUsername':
        'Revolut kullanıcı adınızı girin.',
    'bankName': 'Banka adı',
    'enterBankName': 'Banka adınızı girin.',
    'iban': 'IBAN',
    'ibanOptional':
        'Banka hesap numarası verirseniz isteğe bağlıdır',
    'bankAccountNumber': 'Banka hesap numarası',
    'bankRequiredIfNoIban':
        'IBAN girmezseniz zorunludur',
    'enterIbanOrBank':
        'Bir IBAN veya banka hesap numarası girin.',
    'submitRequest': 'İstek gönder',
    'payoutRequestSubmitted': 'Ödeme isteği gönderildi.',
    'payoutHistoryTitle': 'Ödeme geçmişi',
    'unableLoadPayoutHistory':
        'Ödeme geçmişi şu anda yüklenemiyor.',
    'currencyLabel': 'Para birimi: {currency}',
    'accountHolderLabel': 'Hesap sahibi: {value}',
    'notProvided': 'Sağlanmadı',
    'helpSupport': 'Yardım ve destek',
    'openSupportTicket': 'Bir destek talebi aç',
    'describeIssueAdminReply':
        'Sorununuzu açıklayın, yönetici gelen kutunuzda yanıt verebilir.',
    'subject': 'Konu',
    'helpSubjectHint': 'Hangi konuda yardıma ihtiyacınız var?',
    'message': 'Mesaj',
    'messageHint': 'Mesajınızı buraya yazın...',
    'send': 'Gönder',
    'sending': 'Gönderiliyor...',
    'openInbox': 'Gelen kutusunu aç',
    'yourTickets': 'Talepleriniz',
    'noSupportTicketsYet': 'Henüz destek talebi yok.',
    'supportMessageSent': 'Destek mesajı gönderildi.',
    'adminReply': 'Yönetici yanıtı: {value}',
    'reportBug': 'Hata bildir',
    'tellUsWhatHappened': 'Ne olduğunu bize anlatın',
    'includeStepsExpected':
        'Yeniden oluşturma adımlarını ve görmeyi beklediğiniz sonucu ekleyin.',
    'title': 'Başlık',
    'shortSummary': 'Kısa özet',
    'description': 'Açıklama',
    'describeBug': 'Hatayı açıklayın...',
    'submit': 'Gönder',
    'bugReportSubmitted': 'Hata raporu gönderildi.',
    'aboutTagline':
        'Premium koyu tema • Neon yeşil arayüz',
    'version': 'Sürüm',
    'minimumPayoutLabel': 'Minimum ödeme',
    'processingTimeLabel': 'İşlem süresi',
    'reviewLabel': 'İnceleme',
    'estimatedEarningsOnlyPolicies':
        'Yalnızca tahmini kazançlar. Gerçek kazançlar reklam performansına ve politika kurallarına göre değişebilir.',
    'privacyPolicy': 'Gizlilik politikası',
    'termsOfService': 'Hizmet şartları',
    'notifications': 'Bildirimler',
    'enableNotifications': 'Bildirimleri etkinleştir',
    'generalAppNotifications':
        'Genel uygulama bildirimleri',
    'dailyReminder': 'Günlük hatırlatma',
    'dailyReminderSubtitle':
        'Günlük bonusu tamamlamanız için hatırlatma alın',
    'settingsSaved': 'Ayarlar kaydedildi.',
    'privacy': 'Gizlilik',
    'appVersion': 'Uygulama sürümü',
    'save': 'Kaydet',
    'saving': 'Kaydediliyor...',
    'termsUsingTitle': 'VideoMoney kullanımı',
    'termsViewsTitle': 'Görüntülemeler ve ödüller',
    'termsPayoutsTitle': 'Ödemeler ve inceleme',
    'termsSupportTitle': 'Destek ve mesajlar',
    'termsUsingBullet1':
        'Doğru hesap bilgileri kullanmalısınız.',
    'termsUsingBullet2':
        'Bir kişi ek görüntüleme oluşturmak için birden fazla hesap, bot, komut dosyası, VPN döndürme veya emülatör çiftliklerini kötüye kullanamaz.',
    'termsUsingBullet3':
        'Ödüllü reklamlar, Firebase kimlik doğrulaması ve ödeme incelemesi mevcut platform kurulumu tarafından korunmaya devam eder.',
    'termsViewsBullet1':
        'Uygulamada gösterilen görüntülemeler, VideoMoney içinde kullanılan promosyon ödül birimleridir.',
    'termsViewsBullet2':
        'Tahmini kazançlar yalnızca bilgilendirme amaçlıdır ve platform performansı, politika, dolandırıcılık kontrolleri ve ödeme incelemesine göre değişebilir.',
    'termsViewsBullet3':
        'Günlük bonus ödülleri uygun etkinlikle sınırlıdır ve kötüye kullanım tespit edilirse kaldırılabilir.',
    'termsPayoutsBullet1':
        'Minimum ödeme 10.000 görüntüleme olarak kalır.',
    'termsPayoutsBullet2':
        'Tüm ödeme istekleri manuel yönetici onayı gerektirir ve onaylanabilir, reddedilebilir veya ödendi olarak işaretlenebilir.',
    'termsPayoutsBullet3':
        'Reddedilen ödeme istekleri, yönetici iş akışı izin verdiğinde kullanıcı bakiyesine iade edilebilir.',
    'termsSupportBullet1':
        'Yardım ve destek, hata raporları, yönetici yanıtları ve anlık bildirimler uygulama içi gelen kutunuzda saklanabilir.',
    'termsSupportBullet2':
        'Bildirimleri etkinleştirerek, VideoMoney uygulama güncellemeleri, destek yanıtları ve günlük hatırlatma mesajlarını cihazınıza gönderebilir.',
    'termsSupportBullet3':
        'Ciddi kötüye kullanım, taciz veya dolandırıcılık etkinliği uygulama erişiminin kısıtlanmasına yol açabilir.',
    'settingsTitle': 'Ayarlar',
    'statusPending': 'BEKLİYOR',
    'statusApproved': 'ONAYLANDI',
    'statusPaid': 'ÖDENDİ',
    'statusRejected': 'REDDEDİLDİ',
    'statusProcessing': 'İŞLENİYOR',
    'statusFixed': 'DÜZELTİLDİ',
    'statusClosed': 'KAPATILDI',
    'typeSupport': 'DESTEK',
    'typePayment': 'ÖDEME',
    'typeBug': 'HATA',
  },
  'ar': {
    'appName': 'VideoMoney',
    'startingApp': 'جارٍ تشغيل VideoMoney',
    'preparingStartup':
        'جارٍ تجهيز Firebase وبيانات المحفظة والإعلانات بمكافأة...',
    'startupFailed': 'فشل بدء التشغيل',
    'loadingWallet': 'جارٍ تحميل محفظتك...',
    'unableRestoreSession': 'تعذر استعادة جلستك الآن.',
    'welcomeBack': 'مرحبًا بعودتك',
    'createYourAccount': 'أنشئ حسابك',
    'signInBody':
        'سجّل الدخول لمواصلة كسب المشاهدات وإدارة عمليات السحب.',
    'signUpBody':
        'سجّل لبدء مشاهدة الفيديوهات بمكافأة وبناء رصيدك.',
    'login': 'تسجيل الدخول',
    'signUp': 'إنشاء حساب',
    'email': 'البريد الإلكتروني',
    'password': 'كلمة المرور',
    'enterYourEmail': 'أدخل بريدك الإلكتروني.',
    'enterValidEmail': 'أدخل عنوان بريد إلكتروني صالح.',
    'enterYourPassword': 'أدخل كلمة المرور.',
    'useAtLeast6Chars': 'استخدم 6 أحرف على الأقل.',
    'rememberMe': 'تذكرني',
    'forgotPassword': 'هل نسيت كلمة المرور؟',
    'resetPassword': 'إعادة تعيين كلمة المرور',
    'emailHint': 'name@example.com',
    'cancel': 'إلغاء',
    'sendResetLink': 'إرسال رابط إعادة التعيين',
    'passwordResetEmailSent':
        'تم إرسال رسالة إعادة تعيين كلمة المرور إلى {email}.',
    'unableSendResetEmail':
        'تعذر إرسال رسالة إعادة تعيين كلمة المرور.',
    'firebaseAuthNotice':
        'يبقى Firebase Authentication دون تغيير. هذا التصميم الجديد يحدّث طبقة العرض فقط.',
    'createAccount': 'إنشاء حساب',
    'secureAccessDashboard': 'وصول آمن إلى لوحة أرباحك',
    'firestoreBalanceStored':
        'يتم تخزين سجل السحب ورصيد المكافآت الخاص بك في Firestore',
    'authenticationFailed': 'فشلت المصادقة.',
    'noUserSessionFound': 'لم يتم العثور على جلسة مستخدم.',
    'home': 'الرئيسية',
    'earn': 'اكسب',
    'wallet': 'المحفظة',
    'profile': 'الملف الشخصي',
    'rewardConfirmedViewsAdded':
        'تم تأكيد المكافأة. تمت إضافة المشاهدات.',
    'rewardedAdNotCompleted':
        'لم يتم إكمال الإعلان بمكافأة.',
    'watchVideo': 'شاهد الفيديو',
    'loading': 'جارٍ التحميل...',
    'welcomeBackShort': 'مرحبًا بعودتك،',
    'signedInUser': 'المستخدم المسجل دخوله',
    'watchVideosEarnPaid':
        'شاهد الفيديوهات، اكسب المشاهدات، واحصل على المال.',
    'currentViews': 'المشاهدات الحالية',
    'videosWatched': 'الفيديوهات التي تمت مشاهدتها',
    'progressToPayout': 'التقدم نحو السحب',
    'youAreOnYourWay': 'أنت على الطريق الصحيح.',
    'earnViewsNow': 'اكسب المشاهدات الآن.',
    'payoutUnlocked':
        'تم فتح السحب. يمكنك طلب السحب من المحفظة.',
    'moreViewsUntilPayout':
        'متبقي {views} مشاهدة حتى السحب.',
    'dailyBonus': 'المكافأة اليومية',
    'watchDailyVideosBonus':
        'شاهد {videos} فيديوهات يوميًا للحصول على مشاهدات إضافية.',
    'bonusClaimed': 'تم استلام المكافأة',
    'bonus': 'مكافأة',
    'earnViewsTitle': 'اكسب المشاهدات',
    'watchRewardedEarnViews':
        'شاهد الفيديوهات بمكافأة واكسب المشاهدات فورًا.',
    'earnViews': 'اكسب المشاهدات',
    'howItWorks': 'كيف يعمل',
    'watch': 'شاهد',
    'watchShortVideo': 'شاهد فيديو قصير',
    'earnStep': 'اكسب',
    'getViewsReward': 'احصل على المشاهدات كمكافأة',
    'cashOut': 'سحب الأرباح',
    'reachViews': 'صل إلى {views} مشاهدة',
    'dailyChallenge': 'التحدي اليومي',
    'watchTodayVideosBonus':
        'شاهد {videos} فيديوهات اليوم واحصل على مشاهدات إضافية!',
    'totalVideosWatched': 'إجمالي الفيديوهات المشاهدة',
    'memberSince': 'عضو منذ: {date}',
    'notAvailableYet': 'غير متاح بعد',
    'emailVerified': 'تم التحقق من البريد الإلكتروني',
    'emailNotVerified': 'البريد الإلكتروني غير موثق',
    'security': 'الأمان',
    'firebaseProtected': 'محمي بواسطة Firebase',
    'payoutReview': 'مراجعة السحب',
    'adminApproval': 'موافقة المشرف',
    'readAdminReplies': 'اقرأ ردود المشرف والإشعارات',
    'notificationsPrivacy':
        'الإشعارات والخصوصية وإعدادات التطبيق',
    'contactAdmin': 'تواصل مع المشرف وأرسل الرسائل',
    'reportProblemBug': 'الإبلاغ عن مشكلة أو خطأ',
    'rateApp': 'قيّم التطبيق',
    'sendInAppRating':
        'أرسل تقييمك داخل التطبيق من 1 إلى 5 نجوم',
    'aboutVideoMoney': 'حول VideoMoney',
    'reviewPayoutRequests': 'مراجعة طلبات السحب',
    'logout': 'تسجيل الخروج',
    'yourWallet': 'محفظتك',
    'availableViews': 'المشاهدات المتاحة',
    'estimatedPayout': 'السحب التقديري',
    'remainingToPayout': 'المتبقي للسحب',
    'viewsUnit': 'مشاهدة',
    'estimateOnly':
        'تقدير فقط. 50 مشاهدة ≈ €0.01 وقد تختلف الأرباح الفعلية.',
    'requestPayout': 'طلب سحب',
    'minPayout': 'الحد الأدنى للسحب',
    'processingTime': 'مدة المعالجة',
    'approval': 'الموافقة',
    'adminReview': 'مراجعة المشرف',
    'payoutMethods': 'طرق السحب',
    'payoutHistory': 'سجل السحب',
    'noPayoutRequestsYet': 'لا توجد طلبات سحب بعد.',
    'pendingTimestamp': 'الوقت قيد الانتظار',
    'paypalSubtitle':
        'اطلب السحب بعملات EUR أو GBP أو USD',
    'revolutSubtitle':
        'سحب سريع إلى المحفظة بالعملة التي تختارها',
    'bankTransferTitle': 'تحويل بنكي',
    'bankTransferSubtitle':
        'أضف IBAN أو رقم الحساب البنكي للسحب اليدوي',
    'viewFullHistory': 'عرض السجل الكامل',
    'inbox': 'الوارد',
    'markAllRead': 'تحديد الكل كمقروء',
    'noInboxMessagesYet': 'لا توجد رسائل واردة بعد.',
    'newBadge': 'جديد',
    'rateAppQuestion': 'كيف تقيّم VideoMoney؟',
    'choose1to5':
        'اختر بين 1 و5 نجوم. يمكنك تحديث تقييمك لاحقًا.',
    'saveRating': 'حفظ التقييم',
    'thanksForRating': 'شكرًا على تقييمك.',
    'requestPayoutTitle': 'طلب سحب',
    'payoutRules': 'قواعد السحب',
    'minimumPayoutIs':
        'الحد الأدنى للسحب هو {views} مشاهدة.',
    'processingCanTake':
        'قد تستغرق المعالجة حتى {days} أيام بعد موافقة المشرف.',
    'everyRequestReviewed':
        'يتم مراجعة كل طلب من قبل المشرف قبل دفعه.',
    'useBankAddIban':
        'استخدم البنك للتحويل اليدوي وأضف IBAN أو رقم حسابك البنكي.',
    'submitUsingBalance':
        'قدّم طلب سحب باستخدام رصيد المشاهدات الخاص بك.',
    'estimatedEarningsNotGuaranteed':
        'الأرباح تقديرية فقط. 50 مشاهدة مكتملة ≈ €0.01 وهذا ليس وعدًا مضمونًا بالسحب.',
    'payoutCurrency': 'عملة السحب',
    'payoutMethod': 'طريقة السحب',
    'viewsToRequest': 'المشاهدات المطلوب سحبها',
    'minimumViewsHelper': 'الحد الأدنى 10,000 مشاهدة',
    'enterAmount': 'أدخل مبلغًا.',
    'enterValidPositiveNumber':
        'أدخل رقمًا موجبًا صالحًا.',
    'accountHolderName': 'اسم صاحب الحساب',
    'enterAccountHolderName': 'أدخل اسم صاحب الحساب.',
    'paypalEmail': 'بريد PayPal الإلكتروني',
    'enterPaypalEmail': 'أدخل بريد PayPal الإلكتروني.',
    'enterValidPaypalEmail':
        'أدخل بريد PayPal إلكتروني صالح.',
    'revolutUsername': 'اسم مستخدم Revolut',
    'revolutExample': 'مثال: @yourname',
    'enterRevolutUsername':
        'أدخل اسم مستخدم Revolut الخاص بك.',
    'bankName': 'اسم البنك',
    'enterBankName': 'أدخل اسم البنك.',
    'iban': 'IBAN',
    'ibanOptional':
        'اختياري إذا قمت بتقديم رقم حساب بنكي',
    'bankAccountNumber': 'رقم الحساب البنكي',
    'bankRequiredIfNoIban':
        'مطلوب إذا لم تُدخل IBAN',
    'enterIbanOrBank':
        'أدخل IBAN أو رقم الحساب البنكي.',
    'submitRequest': 'إرسال الطلب',
    'payoutRequestSubmitted': 'تم إرسال طلب السحب.',
    'payoutHistoryTitle': 'سجل السحب',
    'unableLoadPayoutHistory':
        'تعذر تحميل سجل السحب الآن.',
    'currencyLabel': 'العملة: {currency}',
    'accountHolderLabel': 'صاحب الحساب: {value}',
    'notProvided': 'غير متوفر',
    'helpSupport': 'المساعدة والدعم',
    'openSupportTicket': 'فتح تذكرة دعم',
    'describeIssueAdminReply':
        'اشرح مشكلتك ويمكن للمشرف الرد عليك في الوارد.',
    'subject': 'الموضوع',
    'helpSubjectHint': 'بماذا تحتاج إلى المساعدة؟',
    'message': 'الرسالة',
    'messageHint': 'اكتب رسالتك هنا...',
    'send': 'إرسال',
    'sending': 'جارٍ الإرسال...',
    'openInbox': 'فتح الوارد',
    'yourTickets': 'تذاكرك',
    'noSupportTicketsYet': 'لا توجد تذاكر دعم بعد.',
    'supportMessageSent': 'تم إرسال رسالة الدعم.',
    'adminReply': 'رد المشرف: {value}',
    'reportBug': 'الإبلاغ عن خطأ',
    'tellUsWhatHappened': 'أخبرنا بما حدث',
    'includeStepsExpected':
        'ضمّن خطوات إعادة المشكلة وما الذي كنت تتوقع رؤيته.',
    'title': 'العنوان',
    'shortSummary': 'ملخص قصير',
    'description': 'الوصف',
    'describeBug': 'صف الخطأ...',
    'submit': 'إرسال',
    'bugReportSubmitted': 'تم إرسال بلاغ الخطأ.',
    'aboutTagline': 'نسق داكن مميز • واجهة خضراء نيون',
    'version': 'الإصدار',
    'minimumPayoutLabel': 'الحد الأدنى للسحب',
    'processingTimeLabel': 'مدة المعالجة',
    'reviewLabel': 'المراجعة',
    'estimatedEarningsOnlyPolicies':
        'الأرباح تقديرية فقط. قد تختلف الأرباح الفعلية بناءً على أداء الإعلانات وقواعد السياسات.',
    'privacyPolicy': 'سياسة الخصوصية',
    'termsOfService': 'شروط الخدمة',
    'notifications': 'الإشعارات',
    'enableNotifications': 'تفعيل الإشعارات',
    'generalAppNotifications':
        'إشعارات التطبيق العامة',
    'dailyReminder': 'تذكير يومي',
    'dailyReminderSubtitle':
        'احصل على تذكير لإكمال المكافأة اليومية',
    'settingsSaved': 'تم حفظ الإعدادات.',
    'privacy': 'الخصوصية',
    'appVersion': 'إصدار التطبيق',
    'save': 'حفظ',
    'saving': 'جارٍ الحفظ...',
    'termsUsingTitle': 'استخدام VideoMoney',
    'termsViewsTitle': 'المشاهدات والمكافآت',
    'termsPayoutsTitle': 'السحب والمراجعة',
    'termsSupportTitle': 'الدعم والرسائل',
    'termsUsingBullet1':
        'يجب عليك استخدام معلومات حساب دقيقة.',
    'termsUsingBullet2':
        'لا يجوز للشخص الواحد إساءة استخدام حسابات متعددة أو الروبوتات أو السكربتات أو تدوير VPN أو مزارع المحاكيات لتوليد مشاهدات إضافية.',
    'termsUsingBullet3':
        'تظل الإعلانات بمكافأة ومصادقة Firebase ومراجعة السحب محمية من خلال إعداد المنصة الحالي.',
    'termsViewsBullet1':
        'المشاهدات المعروضة في التطبيق هي وحدات مكافأة ترويجية تُستخدم داخل VideoMoney.',
    'termsViewsBullet2':
        'الأرباح التقديرية هي لأغراض إعلامية فقط ويمكن أن تتغير بناءً على أداء المنصة والسياسات وفحوصات الاحتيال ومراجعة السحب.',
    'termsViewsBullet3':
        'مكافآت البونص اليومي تقتصر على النشاط المؤهل ويمكن إزالتها إذا تم اكتشاف إساءة استخدام.',
    'termsPayoutsBullet1':
        'يبقى الحد الأدنى للسحب 10,000 مشاهدة.',
    'termsPayoutsBullet2':
        'تتطلب جميع طلبات السحب موافقة يدوية من المشرف ويمكن الموافقة عليها أو رفضها أو وضع علامة مدفوعة عليها.',
    'termsPayoutsBullet3':
        'قد تتم إعادة طلبات السحب المرفوضة إلى رصيد المستخدم عندما يسمح سير عمل المشرف بذلك.',
    'termsSupportBullet1':
        'يمكن تخزين المساعدة والدعم وبلاغات الأخطاء وردود المشرف والإشعارات الفورية في بريدك الوارد داخل التطبيق.',
    'termsSupportBullet2':
        'بتفعيل الإشعارات، فإنك تسمح لـ VideoMoney بإرسال تحديثات التطبيق وردود الدعم ورسائل التذكير اليومية إلى جهازك.',
    'termsSupportBullet3':
        'قد يؤدي سوء الاستخدام الجسيم أو المضايقة أو النشاط الاحتيالي إلى تقييد الوصول إلى التطبيق.',
    'settingsTitle': 'الإعدادات',
    'statusPending': 'قيد الانتظار',
    'statusApproved': 'تمت الموافقة',
    'statusPaid': 'تم الدفع',
    'statusRejected': 'مرفوض',
    'statusProcessing': 'قيد المعالجة',
    'statusFixed': 'تم الإصلاح',
    'statusClosed': 'مغلق',
    'typeSupport': 'الدعم',
    'typePayment': 'الدفع',
    'typeBug': 'خلل',
  },
  'bn': {
    'appName': 'ভিডিওমানি',
    'startingApp': 'ভিডিওমানি শুরু হচ্ছে',
    'preparingStartup':
        'Firebase, ওয়ালেট ডেটা এবং রিওয়ার্ডেড বিজ্ঞাপন প্রস্তুত করা হচ্ছে...',
    'startupFailed': 'শুরু করতে ব্যর্থ হয়েছে',
    'loadingWallet': 'আপনার ওয়ালেট লোড হচ্ছে...',
    'unableRestoreSession':
        'এই মুহূর্তে আপনার সেশন পুনরুদ্ধার করা যাচ্ছে না।',
    'welcomeBack': 'আবার স্বাগতম',
    'createYourAccount': 'আপনার অ্যাকাউন্ট তৈরি করুন',
    'signInBody':
        'ভিউ উপার্জন চালিয়ে যেতে এবং পেআউট পরিচালনা করতে সাইন ইন করুন।',
    'signUpBody':
        'রিওয়ার্ডেড ভিডিও দেখা শুরু করতে এবং আপনার ব্যালেন্স বাড়াতে নিবন্ধন করুন।',
    'login': 'লগইন',
    'signUp': 'সাইন আপ',
    'email': 'ইমেইল',
    'password': 'পাসওয়ার্ড',
    'enterYourEmail': 'আপনার ইমেইল লিখুন।',
    'enterValidEmail': 'একটি বৈধ ইমেইল ঠিকানা লিখুন।',
    'enterYourPassword': 'আপনার পাসওয়ার্ড লিখুন।',
    'useAtLeast6Chars': 'কমপক্ষে ৬টি অক্ষর ব্যবহার করুন।',
    'rememberMe': 'আমাকে মনে রাখুন',
    'forgotPassword': 'পাসওয়ার্ড ভুলে গেছেন?',
    'resetPassword': 'পাসওয়ার্ড রিসেট করুন',
    'emailHint': 'name@example.com',
    'cancel': 'বাতিল',
    'sendResetLink': 'রিসেট লিংক পাঠান',
    'passwordResetEmailSent':
        '{email}-এ পাসওয়ার্ড রিসেট ইমেইল পাঠানো হয়েছে।',
    'unableSendResetEmail': 'রিসেট ইমেইল পাঠানো যায়নি।',
    'firebaseAuthNotice':
        'Firebase Authentication অপরিবর্তিত থাকবে। এই পুনরায় নকশা শুধু উপস্থাপনা স্তর আপডেট করে।',
    'createAccount': 'অ্যাকাউন্ট তৈরি করুন',
    'secureAccessDashboard':
        'আপনার আয়ের ড্যাশবোর্ডে নিরাপদ প্রবেশাধিকার',
    'firestoreBalanceStored':
        'আপনার পেআউট ইতিহাস এবং রিওয়ার্ড ব্যালেন্স Firestore-এ সংরক্ষিত থাকে',
    'authenticationFailed': 'প্রমাণীকরণ ব্যর্থ হয়েছে।',
    'noUserSessionFound': 'কোনো ব্যবহারকারী সেশন পাওয়া যায়নি।',
    'home': 'হোম',
    'earn': 'আয় করুন',
    'wallet': 'ওয়ালেট',
    'profile': 'প্রোফাইল',
    'rewardConfirmedViewsAdded':
        'পুরস্কার নিশ্চিত হয়েছে। ভিউ যোগ করা হয়েছে।',
    'rewardedAdNotCompleted':
        'রিওয়ার্ডেড বিজ্ঞাপন সম্পূর্ণ করা হয়নি।',
    'watchVideo': 'ভিডিও দেখুন',
    'loading': 'লোড হচ্ছে...',
    'welcomeBackShort': 'আবার স্বাগতম,',
    'signedInUser': 'সাইন-ইন করা ব্যবহারকারী',
    'watchVideosEarnPaid':
        'ভিডিও দেখুন, ভিউ উপার্জন করুন, এবং অর্থ পান।',
    'currentViews': 'বর্তমান ভিউ',
    'videosWatched': 'দেখা ভিডিও',
    'progressToPayout': 'পেআউটের অগ্রগতি',
    'youAreOnYourWay': 'আপনি সঠিক পথে আছেন।',
    'earnViewsNow': 'এখনই ভিউ আয় করুন।',
    'payoutUnlocked':
        'পেআউট আনলক হয়েছে। আপনি Wallet-এ পেআউট অনুরোধ করতে পারেন।',
    'moreViewsUntilPayout':
        'পেআউট পর্যন্ত আরও {views} ভিউ বাকি।',
    'dailyBonus': 'দৈনিক বোনাস',
    'watchDailyVideosBonus':
        'দৈনিক বোনাস পেতে প্রতিদিন {videos}টি ভিডিও দেখুন।',
    'bonusClaimed': 'বোনাস গ্রহণ করা হয়েছে',
    'bonus': 'বোনাস',
    'earnViewsTitle': 'ভিউ আয় করুন',
    'watchRewardedEarnViews':
        'রিওয়ার্ডেড ভিডিও দেখুন এবং সঙ্গে সঙ্গে ভিউ আয় করুন।',
    'earnViews': 'ভিউ আয় করুন',
    'howItWorks': 'এটি কীভাবে কাজ করে',
    'watch': 'দেখুন',
    'watchShortVideo': 'একটি ছোট ভিডিও দেখুন',
    'earnStep': 'আয় করুন',
    'getViewsReward': 'পুরস্কার হিসেবে ভিউ পান',
    'cashOut': 'ক্যাশ আউট',
    'reachViews': '{views} ভিউতে পৌঁছান',
    'dailyChallenge': 'দৈনিক চ্যালেঞ্জ',
    'watchTodayVideosBonus':
        'আজ {videos}টি ভিডিও দেখুন এবং বোনাস ভিউ পান!',
    'totalVideosWatched': 'মোট দেখা ভিডিও',
    'memberSince': 'সদস্য হয়েছেন: {date}',
    'notAvailableYet': 'এখনও উপলব্ধ নয়',
    'emailVerified': 'ইমেইল যাচাইকৃত',
    'emailNotVerified': 'ইমেইল যাচাইকৃত নয়',
    'security': 'নিরাপত্তা',
    'firebaseProtected': 'Firebase দ্বারা সুরক্ষিত',
    'payoutReview': 'পেআউট পর্যালোচনা',
    'adminApproval': 'অ্যাডমিন অনুমোদন',
    'readAdminReplies':
        'অ্যাডমিনের উত্তর এবং নোটিফিকেশন পড়ুন',
    'notificationsPrivacy':
        'নোটিফিকেশন, গোপনীয়তা এবং অ্যাপ সেটিংস',
    'contactAdmin':
        'অ্যাডমিনের সাথে যোগাযোগ করুন এবং বার্তা পাঠান',
    'reportProblemBug': 'সমস্যা বা বাগ রিপোর্ট করুন',
    'rateApp': 'অ্যাপ রেট করুন',
    'sendInAppRating':
        'অ্যাপের মধ্যে ১ থেকে ৫ তারকা পর্যন্ত আপনার রেটিং দিন',
    'aboutVideoMoney': 'ভিডিওমানি সম্পর্কে',
    'reviewPayoutRequests': 'পেআউট অনুরোধসমূহ পর্যালোচনা করুন',
    'logout': 'লগআউট',
    'yourWallet': 'আপনার ওয়ালেট',
    'availableViews': 'উপলব্ধ ভিউ',
    'estimatedPayout': 'আনুমানিক পেআউট',
    'remainingToPayout': 'পেআউটের জন্য বাকি',
    'viewsUnit': 'ভিউ',
    'estimateOnly':
        'এটি শুধু আনুমানিক। ৫০ ভিউ ≈ €0.01 এবং প্রকৃত আয় ভিন্ন হতে পারে।',
    'requestPayout': 'পেআউট অনুরোধ করুন',
    'minPayout': 'সর্বনিম্ন পেআউট',
    'processingTime': 'প্রক্রিয়াকরণের সময়',
    'approval': 'অনুমোদন',
    'adminReview': 'অ্যাডমিন পর্যালোচনা',
    'payoutMethods': 'পেআউট পদ্ধতি',
    'payoutHistory': 'পেআউট ইতিহাস',
    'noPayoutRequestsYet': 'এখনও কোনো পেআউট অনুরোধ নেই।',
    'pendingTimestamp': 'মুলতুবি টাইমস্ট্যাম্প',
    'paypalSubtitle':
        'EUR, GBP, অথবা USD-এ পেআউট অনুরোধ করুন',
    'revolutSubtitle':
        'নির্বাচিত মুদ্রায় দ্রুত ওয়ালেট পেআউট',
    'bankTransferTitle': 'ব্যাংক ট্রান্সফার',
    'bankTransferSubtitle':
        'ম্যানুয়াল পেআউটের জন্য IBAN বা ব্যাংক অ্যাকাউন্ট নম্বর যোগ করুন',
    'viewFullHistory': 'সম্পূর্ণ ইতিহাস দেখুন',
    'inbox': 'ইনবক্স',
    'markAllRead': 'সব পড়া হিসেবে চিহ্নিত করুন',
    'noInboxMessagesYet': 'এখনও কোনো ইনবক্স বার্তা নেই।',
    'newBadge': 'নতুন',
    'rateAppQuestion': 'আপনি ভিডিওমানিকে কীভাবে রেট করবেন?',
    'choose1to5':
        '১ থেকে ৫ তারকার মধ্যে নির্বাচন করুন। পরে চাইলে আপনার রেটিং আপডেট করতে পারবেন।',
    'saveRating': 'রেটিং সংরক্ষণ করুন',
    'thanksForRating': 'আপনার রেটিংয়ের জন্য ধন্যবাদ।',
    'requestPayoutTitle': 'পেআউট অনুরোধ',
    'payoutRules': 'পেআউটের নিয়ম',
    'minimumPayoutIs': 'সর্বনিম্ন পেআউট হলো {views} ভিউ।',
    'processingCanTake':
        'অ্যাডমিন অনুমোদনের পরে প্রক্রিয়াকরণে সর্বোচ্চ {days} দিন লাগতে পারে।',
    'everyRequestReviewed':
        'প্রতিটি অনুরোধ অর্থ প্রদান করার আগে অ্যাডমিন দ্বারা পর্যালোচিত হয়।',
    'useBankAddIban':
        'ম্যানুয়াল ট্রান্সফারের জন্য Bank ব্যবহার করুন এবং আপনার IBAN বা ব্যাংক অ্যাকাউন্ট নম্বর যোগ করুন।',
    'submitUsingBalance':
        'আপনার ভিউ ব্যালেন্স ব্যবহার করে একটি পেআউট অনুরোধ জমা দিন।',
    'estimatedEarningsNotGuaranteed':
        'আনুমানিক আয় মাত্র। ৫০টি সম্পূর্ণ ভিউ ≈ €0.01 এবং এটি নিশ্চিত পেআউটের প্রতিশ্রুতি নয়।',
    'payoutCurrency': 'পেআউটের মুদ্রা',
    'payoutMethod': 'পেআউট পদ্ধতি',
    'viewsToRequest': 'অনুরোধের জন্য ভিউ',
    'minimumViewsHelper': 'সর্বনিম্ন ১০,০০০ ভিউ',
    'enterAmount': 'একটি পরিমাণ লিখুন।',
    'enterValidPositiveNumber':
        'একটি বৈধ ধনাত্মক সংখ্যা লিখুন।',
    'accountHolderName': 'অ্যাকাউন্টধারীর নাম',
    'enterAccountHolderName':
        'অ্যাকাউন্টধারীর নাম লিখুন।',
    'paypalEmail': 'পেপ্যাল ইমেইল',
    'enterPaypalEmail': 'একটি পেপ্যাল ইমেইল লিখুন।',
    'enterValidPaypalEmail':
        'একটি বৈধ পেপ্যাল ইমেইল লিখুন।',
    'revolutUsername': 'রেভলুট ব্যবহারকারীর নাম',
    'revolutExample': 'উদাহরণ: @yourname',
    'enterRevolutUsername':
        'আপনার রেভলুট ব্যবহারকারীর নাম লিখুন।',
    'bankName': 'ব্যাংকের নাম',
    'enterBankName': 'আপনার ব্যাংকের নাম লিখুন।',
    'iban': 'IBAN',
    'ibanOptional':
        'আপনি যদি ব্যাংক অ্যাকাউন্ট নম্বর দেন, তাহলে এটি ঐচ্ছিক',
    'bankAccountNumber': 'ব্যাংক অ্যাকাউন্ট নম্বর',
    'bankRequiredIfNoIban':
        'আপনি যদি IBAN না দেন, তাহলে এটি আবশ্যক',
    'enterIbanOrBank':
        'একটি IBAN বা ব্যাংক অ্যাকাউন্ট নম্বর লিখুন।',
    'submitRequest': 'অনুরোধ জমা দিন',
    'payoutRequestSubmitted': 'পেআউট অনুরোধ জমা দেওয়া হয়েছে।',
    'payoutHistoryTitle': 'পেআউট ইতিহাস',
    'unableLoadPayoutHistory':
        'এই মুহূর্তে পেআউট ইতিহাস লোড করা যাচ্ছে না।',
    'currencyLabel': 'মুদ্রা: {currency}',
    'accountHolderLabel': 'অ্যাকাউন্টধারী: {value}',
    'notProvided': 'দেওয়া হয়নি',
    'helpSupport': 'সহায়তা ও সাপোর্ট',
    'openSupportTicket': 'একটি সাপোর্ট টিকিট খুলুন',
    'describeIssueAdminReply':
        'আপনার সমস্যাটি বর্ণনা করুন এবং অ্যাডমিন আপনার ইনবক্সে উত্তর দিতে পারবেন।',
    'subject': 'বিষয়',
    'helpSubjectHint': 'আপনার কী বিষয়ে সাহায্য দরকার?',
    'message': 'বার্তা',
    'messageHint': 'এখানে আপনার বার্তা লিখুন...',
    'send': 'পাঠান',
    'sending': 'পাঠানো হচ্ছে...',
    'openInbox': 'ইনবক্স খুলুন',
    'yourTickets': 'আপনার টিকিটসমূহ',
    'noSupportTicketsYet': 'এখনও কোনো সাপোর্ট টিকিট নেই।',
    'supportMessageSent': 'সাপোর্ট বার্তা পাঠানো হয়েছে।',
    'adminReply': 'অ্যাডমিনের উত্তর: {value}',
    'reportBug': 'বাগ রিপোর্ট করুন',
    'tellUsWhatHappened': 'কি ঘটেছে আমাদের জানান',
    'includeStepsExpected':
        'পুনরায় ঘটানোর ধাপ এবং আপনি কী দেখতে চেয়েছিলেন তা অন্তর্ভুক্ত করুন।',
    'title': 'শিরোনাম',
    'shortSummary': 'সংক্ষিপ্ত সারাংশ',
    'description': 'বিবরণ',
    'describeBug': 'বাগটি বর্ণনা করুন...',
    'submit': 'জমা দিন',
    'bugReportSubmitted': 'বাগ রিপোর্ট জমা দেওয়া হয়েছে।',
    'aboutTagline':
        'প্রিমিয়াম ডার্ক থিম • নিয়ন সবুজ UI',
    'version': 'সংস্করণ',
    'minimumPayoutLabel': 'সর্বনিম্ন পেআউট',
    'processingTimeLabel': 'প্রক্রিয়াকরণের সময়',
    'reviewLabel': 'পর্যালোচনা',
    'estimatedEarningsOnlyPolicies':
        'আনুমানিক আয় মাত্র। বিজ্ঞাপনের কার্যকারিতা এবং নীতিমালার ভিত্তিতে প্রকৃত আয় ভিন্ন হতে পারে।',
    'privacyPolicy': 'গোপনীয়তা নীতি',
    'termsOfService': 'সেবার শর্তাবলি',
    'notifications': 'নোটিফিকেশন',
    'enableNotifications': 'নোটিফিকেশন চালু করুন',
    'generalAppNotifications': 'সাধারণ অ্যাপ নোটিফিকেশন',
    'dailyReminder': 'দৈনিক অনুস্মারক',
    'dailyReminderSubtitle':
        'দৈনিক বোনাস সম্পূর্ণ করার জন্য একটি অনুস্মারক পান',
    'settingsSaved': 'সেটিংস সংরক্ষিত হয়েছে।',
    'privacy': 'গোপনীয়তা',
    'appVersion': 'অ্যাপের সংস্করণ',
    'save': 'সংরক্ষণ করুন',
    'saving': 'সংরক্ষণ করা হচ্ছে...',
    'termsUsingTitle': 'ভিডিওমানি ব্যবহার',
    'termsViewsTitle': 'ভিউ এবং পুরস্কার',
    'termsPayoutsTitle': 'পেআউট এবং পর্যালোচনা',
    'termsSupportTitle': 'সাপোর্ট এবং বার্তা',
    'termsUsingBullet1':
        'আপনাকে সঠিক অ্যাকাউন্ট তথ্য ব্যবহার করতে হবে।',
    'termsUsingBullet2':
        'অতিরিক্ত ভিউ তৈরির জন্য একজন ব্যক্তি একাধিক অ্যাকাউন্ট, বট, স্ক্রিপ্ট, VPN রোটেশন, বা এমুলেটর ফার্মের অপব্যবহার করতে পারবেন না।',
    'termsUsingBullet3':
        'রিওয়ার্ডেড বিজ্ঞাপন, Firebase authentication, এবং পেআউট পর্যালোচনা বিদ্যমান প্ল্যাটফর্ম সেটআপ দ্বারা সুরক্ষিত থাকবে।',
    'termsViewsBullet1':
        'অ্যাপে প্রদর্শিত ভিউ হলো VideoMoney-এর ভেতরে ব্যবহৃত প্রচারণামূলক পুরস্কার একক।',
    'termsViewsBullet2':
        'আনুমানিক আয় শুধুমাত্র তথ্যগত উদ্দেশ্যে এবং প্ল্যাটফর্মের কার্যকারিতা, নীতি, জালিয়াতি যাচাই, ও পেআউট পর্যালোচনার ভিত্তিতে পরিবর্তিত হতে পারে।',
    'termsViewsBullet3':
        'দৈনিক বোনাস পুরস্কার শুধুমাত্র যোগ্য কার্যকলাপের জন্য সীমাবদ্ধ এবং অপব্যবহার শনাক্ত হলে তা সরিয়ে দেওয়া হতে পারে।',
    'termsPayoutsBullet1':
        'সর্বনিম্ন পেআউট ১০,০০০ ভিউই থাকবে।',
    'termsPayoutsBullet2':
        'সব পেআউট অনুরোধে ম্যানুয়াল অ্যাডমিন অনুমোদন প্রয়োজন এবং তা অনুমোদিত, প্রত্যাখ্যাত, অথবা পরিশোধিত হিসেবে চিহ্নিত হতে পারে।',
    'termsPayoutsBullet3':
        'অ্যাডমিন ওয়ার্কফ্লো অনুযায়ী অনুমোদিত হলে প্রত্যাখ্যাত পেআউট অনুরোধ ব্যবহারকারীর ব্যালেন্সে ফেরত দেওয়া হতে পারে।',
    'termsSupportBullet1':
        'সহায়তা ও সাপোর্ট, বাগ রিপোর্ট, অ্যাডমিনের উত্তর, এবং পুশ নোটিফিকেশন আপনার অ্যাপের ইনবক্সে সংরক্ষিত হতে পারে।',
    'termsSupportBullet2':
        'নোটিফিকেশন চালু করার মাধ্যমে আপনি VideoMoney-কে আপনার ডিভাইসে অ্যাপ আপডেট, সাপোর্ট উত্তর, এবং দৈনিক অনুস্মারক বার্তা পাঠানোর অনুমতি দিচ্ছেন।',
    'termsSupportBullet3':
        'গুরুতর অপব্যবহার, হয়রানি, বা প্রতারণামূলক কার্যকলাপের ফলে অ্যাপ ব্যবহারে সীমাবদ্ধতা আসতে পারে।',
    'settingsTitle': 'সেটিংস',
    'statusPending': 'মুলতুবি',
    'statusApproved': 'অনুমোদিত',
    'statusPaid': 'পরিশোধিত',
    'statusRejected': 'প্রত্যাখ্যাত',
    'statusProcessing': 'প্রক্রিয়াধীন',
    'statusFixed': 'সমাধান হয়েছে',
    'statusClosed': 'বন্ধ',
    'typeSupport': 'সাপোর্ট',
    'typePayment': 'পেমেন্ট',
    'typeBug': 'বাগ',
  },
  'ta': {
    'appName': 'VideoMoney',
    'startingApp': 'VideoMoney தொடங்கப்படுகிறது',
    'preparingStartup':
        'Firebase, வாலெட் தரவு, மற்றும் பரிசளிக்கும் விளம்பரங்கள் தயாராக்கப்படுகின்றன...',
    'startupFailed': 'தொடக்கம் தோல்வியடைந்தது',
    'loadingWallet': 'உங்கள் வாலெட் ஏற்றப்படுகிறது...',
    'unableRestoreSession':
        'இப்போது உங்கள் அமர்வை மீட்டெடுக்க முடியவில்லை.',
    'welcomeBack': 'மீண்டும் வரவேற்கிறோம்',
    'createYourAccount': 'உங்கள் கணக்கை உருவாக்குங்கள்',
    'signInBody':
        'தொடர்ந்து பார்வைகள் சம்பாதிக்கவும், கட்டணங்களை நிர்வகிக்கவும் உள்நுழையுங்கள்.',
    'signUpBody':
        'பரிசளிக்கும் வீடியோக்களை பார்க்கவும், உங்கள் இருப்பை உருவாக்கவும் பதிவு செய்யுங்கள்.',
    'login': 'உள்நுழை',
    'signUp': 'பதிவு செய்யுங்கள்',
    'email': 'மின்னஞ்சல்',
    'password': 'கடவுச்சொல்',
    'enterYourEmail': 'உங்கள் மின்னஞ்சலை உள்ளிடுங்கள்.',
    'enterValidEmail':
        'சரியான மின்னஞ்சல் முகவரியை உள்ளிடுங்கள்.',
    'enterYourPassword':
        'உங்கள் கடவுச்சொல்லை உள்ளிடுங்கள்.',
    'useAtLeast6Chars':
        'குறைந்தது 6 எழுத்துகளை பயன்படுத்துங்கள்.',
    'rememberMe': 'என்னை நினைவில் வைத்துக்கொள்ளவும்',
    'forgotPassword': 'கடவுச்சொல் மறந்துவிட்டதா?',
    'resetPassword': 'கடவுச்சொல்லை மீட்டமை',
    'emailHint': 'name@example.com',
    'cancel': 'ரத்து செய்',
    'sendResetLink': 'மீட்டமைப்பு இணைப்பை அனுப்பு',
    'passwordResetEmailSent':
        'கடவுச்சொல் மீட்டமைப்பு மின்னஞ்சல் {email} க்கு அனுப்பப்பட்டது.',
    'unableSendResetEmail':
        'கடவுச்சொல் மீட்டமைப்பு மின்னஞ்சலை அனுப்ப முடியவில்லை.',
    'firebaseAuthNotice':
        'Firebase Authentication மாற்றமின்றி இருக்கும். இந்த மறுவடிவமைப்பு காட்சி அடுக்கை மட்டுமே புதுப்பிக்கிறது.',
    'createAccount': 'கணக்கை உருவாக்கு',
    'secureAccessDashboard':
        'உங்கள் வருமான டாஷ்போர்டுக்கு பாதுகாப்பான அணுகல்',
    'firestoreBalanceStored':
        'உங்கள் கட்டண வரலாறு மற்றும் பரிசு இருப்பு Firestore-ல் சேமிக்கப்படுகிறது',
    'authenticationFailed': 'அங்கீகாரம் தோல்வியடைந்தது.',
    'noUserSessionFound':
        'பயனர் அமர்வு எதுவும் கிடைக்கவில்லை.',
    'home': 'முகப்பு',
    'earn': 'சம்பாதி',
    'wallet': 'வாலெட்',
    'profile': 'சுயவிவரம்',
    'rewardConfirmedViewsAdded':
        'பரிசு உறுதிப்படுத்தப்பட்டது. பார்வைகள் சேர்க்கப்பட்டன.',
    'rewardedAdNotCompleted':
        'பரிசளிக்கும் விளம்பரம் நிறைவுபெறவில்லை.',
    'watchVideo': 'வீடியோவை பார்க்கவும்',
    'loading': 'ஏற்றப்படுகிறது...',
    'welcomeBackShort': 'மீண்டும் வரவேற்கிறோம்,',
    'signedInUser': 'உள்நுழைந்த பயனர்',
    'watchVideosEarnPaid':
        'வீடியோக்களை பாருங்கள், பார்வைகள் சம்பாதியுங்கள், மற்றும் பணம் பெறுங்கள்.',
    'currentViews': 'தற்போதைய பார்வைகள்',
    'videosWatched': 'பார்க்கப்பட்ட வீடியோக்கள்',
    'progressToPayout': 'கட்டணத்திற்கு முன்னேற்றம்',
    'youAreOnYourWay': 'நீங்கள் சரியான பாதையில் உள்ளீர்கள்.',
    'earnViewsNow': 'இப்போது பார்வைகள் சம்பாதியுங்கள்.',
    'payoutUnlocked':
        'கட்டணம் திறக்கப்பட்டுள்ளது. Wallet-ல் இருந்து கட்டணம் கோரலாம்.',
    'moreViewsUntilPayout':
        'கட்டணத்திற்கு இன்னும் {views} பார்வைகள் வேண்டும்.',
    'dailyBonus': 'தினசரி போனஸ்',
    'watchDailyVideosBonus':
        'போனஸ் பார்வைகள் பெற தினமும் {videos} வீடியோக்களை பாருங்கள்.',
    'bonusClaimed': 'போனஸ் பெறப்பட்டது',
    'bonus': 'போனஸ்',
    'earnViewsTitle': 'பார்வைகள் சம்பாதி',
    'watchRewardedEarnViews':
        'பரிசளிக்கும் வீடியோக்களை பார்த்து உடனே பார்வைகள் சம்பாதியுங்கள்.',
    'earnViews': 'பார்வைகள் சம்பாதி',
    'howItWorks': 'இது எப்படி செயல்படுகிறது',
    'watch': 'பார்',
    'watchShortVideo': 'சிறிய வீடியோவை பாருங்கள்',
    'earnStep': 'சம்பாதி',
    'getViewsReward': 'பரிசாக பார்வைகள் பெறுங்கள்',
    'cashOut': 'பணம் எடு',
    'reachViews': '{views} பார்வைகளை அடையுங்கள்',
    'dailyChallenge': 'தினசரி சவால்',
    'watchTodayVideosBonus':
        'இன்று {videos} வீடியோக்களை பார்த்து போனஸ் பார்வைகள் பெறுங்கள்!',
    'totalVideosWatched': 'மொத்தம் பார்க்கப்பட்ட வீடியோக்கள்',
    'memberSince': 'உறுப்பினராக உள்ள தேதி: {date}',
    'notAvailableYet': 'இன்னும் கிடைக்கவில்லை',
    'emailVerified': 'மின்னஞ்சல் சரிபார்க்கப்பட்டது',
    'emailNotVerified':
        'மின்னஞ்சல் சரிபார்க்கப்படவில்லை',
    'security': 'பாதுகாப்பு',
    'firebaseProtected': 'Firebase மூலம் பாதுகாக்கப்பட்டது',
    'payoutReview': 'கட்டண மதிப்பாய்வு',
    'adminApproval': 'நிர்வாகி ஒப்புதல்',
    'readAdminReplies':
        'நிர்வாகி பதில்கள் மற்றும் அறிவிப்புகளை படிக்கவும்',
    'notificationsPrivacy':
        'அறிவிப்புகள், தனியுரிமை, மற்றும் பயன்பாட்டு அமைப்புகள்',
    'contactAdmin':
        'நிர்வாகியை தொடர்புகொண்டு செய்திகளை அனுப்புங்கள்',
    'reportProblemBug':
        'ஒரு பிரச்சனை அல்லது பிழையை தெரிவிக்கவும்',
    'rateApp': 'பயன்பாட்டை மதிப்பிடுங்கள்',
    'sendInAppRating':
        'பயன்பாட்டுக்குள் 1 முதல் 5 நட்சத்திரங்கள் வரை உங்கள் மதிப்பீட்டை அனுப்புங்கள்',
    'aboutVideoMoney': 'VideoMoney பற்றி',
    'reviewPayoutRequests':
        'கட்டண கோரிக்கைகளை மதிப்பாய்வு செய்யுங்கள்',
    'logout': 'வெளியேறு',
    'yourWallet': 'உங்கள் வாலெட்',
    'availableViews': 'கிடைக்கும் பார்வைகள்',
    'estimatedPayout': 'மதிப்பிடப்பட்ட கட்டணம்',
    'remainingToPayout': 'கட்டணத்திற்கு மீதமுள்ளது',
    'viewsUnit': 'பார்வைகள்',
    'estimateOnly':
        'இது ஒரு மதிப்பீடு மட்டுமே. 50 பார்வைகள் ≈ €0.01 மற்றும் உண்மையான வருவாய் மாறுபடலாம்.',
    'requestPayout': 'கட்டணம் கோருங்கள்',
    'minPayout': 'குறைந்தபட்ச கட்டணம்',
    'processingTime': 'செயலாக்க நேரம்',
    'approval': 'ஒப்புதல்',
    'adminReview': 'நிர்வாகி மதிப்பாய்வு',
    'payoutMethods': 'கட்டண முறைகள்',
    'payoutHistory': 'கட்டண வரலாறு',
    'noPayoutRequestsYet':
        'இன்னும் கட்டண கோரிக்கைகள் எதுவும் இல்லை.',
    'pendingTimestamp': 'நிலுவை நேரமுத்திரை',
    'paypalSubtitle':
        'EUR, GBP, அல்லது USD-இல் கட்டணம் கோருங்கள்',
    'revolutSubtitle':
        'தேர்ந்தெடுத்த நாணயத்தில் வேகமான வாலெட் கட்டணம்',
    'bankTransferTitle': 'வங்கி பரிமாற்றம்',
    'bankTransferSubtitle':
        'கைமுறை கட்டணத்திற்காக IBAN அல்லது வங்கி கணக்கு எண்ணை சேர்க்கவும்',
    'viewFullHistory': 'முழு வரலாற்றைப் பார்க்கவும்',
    'inbox': 'இன்பாக்ஸ்',
    'markAllRead':
        'அனைத்தையும் படித்ததாக குறிக்கவும்',
    'noInboxMessagesYet':
        'இன்னும் இன்பாக்ஸ் செய்திகள் எதுவும் இல்லை.',
    'newBadge': 'புதியது',
    'rateAppQuestion':
        'VideoMoney-ஐ நீங்கள் எப்படி மதிப்பிடுவீர்கள்?',
    'choose1to5':
        '1 முதல் 5 நட்சத்திரங்கள் வரை தேர்வு செய்யுங்கள். பின்னர் உங்கள் மதிப்பீட்டை மாற்றலாம்.',
    'saveRating': 'மதிப்பீட்டை சேமி',
    'thanksForRating': 'உங்கள் மதிப்பீட்டுக்கு நன்றி.',
    'requestPayoutTitle': 'கட்டணம் கோருங்கள்',
    'payoutRules': 'கட்டண விதிகள்',
    'minimumPayoutIs':
        'குறைந்தபட்ச கட்டணம் {views} பார்வைகள்.',
    'processingCanTake':
        'நிர்வாகி ஒப்புதலுக்குப் பிறகு செயலாக்கம் அதிகபட்சம் {days} நாட்கள் ஆகலாம்.',
    'everyRequestReviewed':
        'ஒவ்வொரு கோரிக்கையும் கட்டணம் செலுத்தப்படும் முன் நிர்வாகியால் மதிப்பாய்வு செய்யப்படும்.',
    'useBankAddIban':
        'கைமுறை பரிமாற்றத்திற்கு வங்கியை பயன்படுத்தி உங்கள் IBAN அல்லது வங்கி கணக்கு எண்ணை சேர்க்கவும்.',
    'submitUsingBalance':
        'உங்கள் பார்வை இருப்பைப் பயன்படுத்தி கட்டண கோரிக்கையை சமர்ப்பிக்கவும்.',
    'estimatedEarningsNotGuaranteed':
        'மதிப்பிடப்பட்ட வருமானம் மட்டுமே. 50 நிறைவு செய்யப்பட்ட பார்வைகள் ≈ €0.01, இது உத்தரவாதப்பட்ட கட்டண வாக்குறுதி அல்ல.',
    'payoutCurrency': 'கட்டண நாணயம்',
    'payoutMethod': 'கட்டண முறை',
    'viewsToRequest': 'கோர வேண்டிய பார்வைகள்',
    'minimumViewsHelper': 'குறைந்தபட்சம் 10,000 பார்வைகள்',
    'enterAmount': 'ஒரு தொகையை உள்ளிடுங்கள்.',
    'enterValidPositiveNumber':
        'சரியான நேர்ம தொகையை உள்ளிடுங்கள்.',
    'accountHolderName': 'கணக்குதாரர் பெயர்',
    'enterAccountHolderName':
        'கணக்குதாரர் பெயரை உள்ளிடுங்கள்.',
    'paypalEmail': 'PayPal மின்னஞ்சல்',
    'enterPaypalEmail':
        'ஒரு PayPal மின்னஞ்சலை உள்ளிடுங்கள்.',
    'enterValidPaypalEmail':
        'சரியான PayPal மின்னஞ்சலை உள்ளிடுங்கள்.',
    'revolutUsername': 'Revolut பயனர்பெயர்',
    'revolutExample': 'உதாரணம்: @yourname',
    'enterRevolutUsername':
        'உங்கள் Revolut பயனர்பெயரை உள்ளிடுங்கள்.',
    'bankName': 'வங்கியின் பெயர்',
    'enterBankName': 'உங்கள் வங்கியின் பெயரை உள்ளிடுங்கள்.',
    'iban': 'IBAN',
    'ibanOptional':
        'வங்கி கணக்கு எண்ணை வழங்கினால் இது விருப்பத் தேர்வு',
    'bankAccountNumber': 'வங்கி கணக்கு எண்',
    'bankRequiredIfNoIban':
        'நீங்கள் IBAN அளிக்காவிட்டால் இது அவசியம்',
    'enterIbanOrBank':
        'IBAN அல்லது வங்கி கணக்கு எண்ணை உள்ளிடுங்கள்.',
    'submitRequest': 'கோரிக்கையை சமர்ப்பி',
    'payoutRequestSubmitted':
        'கட்டண கோரிக்கை சமர்ப்பிக்கப்பட்டது.',
    'payoutHistoryTitle': 'கட்டண வரலாறு',
    'unableLoadPayoutHistory':
        'இப்போது கட்டண வரலாற்றை ஏற்ற முடியவில்லை.',
    'currencyLabel': 'நாணயம்: {currency}',
    'accountHolderLabel': 'கணக்குதாரர்: {value}',
    'notProvided': 'வழங்கப்படவில்லை',
    'helpSupport': 'உதவி மற்றும் ஆதரவு',
    'openSupportTicket': 'ஆதரவு டிக்கெட்டை திறக்கவும்',
    'describeIssueAdminReply':
        'உங்கள் பிரச்சனையை விளக்குங்கள்; நிர்வாகி உங்கள் இன்பாக்ஸில் பதிலளிக்க முடியும்.',
    'subject': 'பொருள்',
    'helpSubjectHint': 'உங்களுக்கு எந்த உதவி தேவை?',
    'message': 'செய்தி',
    'messageHint': 'உங்கள் செய்தியை இங்கே எழுதுங்கள்...',
    'send': 'அனுப்பு',
    'sending': 'அனுப்பப்படுகிறது...',
    'openInbox': 'இன்பாக்ஸை திறக்கவும்',
    'yourTickets': 'உங்கள் டிக்கெட்டுகள்',
    'noSupportTicketsYet':
        'இன்னும் ஆதரவு டிக்கெட்டுகள் எதுவும் இல்லை.',
    'supportMessageSent': 'ஆதரவு செய்தி அனுப்பப்பட்டது.',
    'adminReply': 'நிர்வாகி பதில்: {value}',
    'reportBug': 'பிழையை தெரிவிக்கவும்',
    'tellUsWhatHappened': 'என்ன நடந்தது என்பதை எங்களிடம் சொல்லுங்கள்',
    'includeStepsExpected':
        'மீண்டும் உருவாக்கும் படிகள் மற்றும் நீங்கள் எதிர்பார்த்ததை சேர்க்கவும்.',
    'title': 'தலைப்பு',
    'shortSummary': 'சுருக்கமான சுருக்கம்',
    'description': 'விளக்கம்',
    'describeBug': 'பிழையை விவரிக்கவும்...',
    'submit': 'சமர்ப்பிக்கவும்',
    'bugReportSubmitted':
        'பிழை அறிக்கை சமர்ப்பிக்கப்பட்டது.',
    'aboutTagline':
        'பிரீமியம் டார்க் தீம் • நீயான் பச்சை UI',
    'version': 'பதிப்பு',
    'minimumPayoutLabel': 'குறைந்தபட்ச கட்டணம்',
    'processingTimeLabel': 'செயலாக்க நேரம்',
    'reviewLabel': 'மதிப்பாய்வு',
    'estimatedEarningsOnlyPolicies':
        'மதிப்பிடப்பட்ட வருமானம் மட்டுமே. விளம்பர செயல்திறன் மற்றும் கொள்கை விதிகளின் அடிப்படையில் உண்மையான வருமானம் மாறுபடலாம்.',
    'privacyPolicy': 'தனியுரிமைக் கொள்கை',
    'termsOfService': 'சேவை விதிமுறைகள்',
    'notifications': 'அறிவிப்புகள்',
    'enableNotifications': 'அறிவிப்புகளை இயக்கவும்',
    'generalAppNotifications':
        'பொது பயன்பாட்டு அறிவிப்புகள்',
    'dailyReminder': 'தினசரி நினைவூட்டல்',
    'dailyReminderSubtitle':
        'தினசரி போனஸை முடிக்க ஒரு நினைவூட்டலை பெறுங்கள்',
    'settingsSaved': 'அமைப்புகள் சேமிக்கப்பட்டன.',
    'privacy': 'தனியுரிமை',
    'appVersion': 'பயன்பாட்டு பதிப்பு',
    'save': 'சேமி',
    'saving': 'சேமிக்கப்படுகிறது...',
    'termsUsingTitle': 'VideoMoney-ஐ பயன்படுத்துவது',
    'termsViewsTitle': 'பார்வைகள் மற்றும் பரிசுகள்',
    'termsPayoutsTitle': 'கட்டணங்கள் மற்றும் மதிப்பாய்வு',
    'termsSupportTitle': 'ஆதரவு மற்றும் செய்திகள்',
    'termsUsingBullet1':
        'நீங்கள் சரியான கணக்கு தகவலை பயன்படுத்த வேண்டும்.',
    'termsUsingBullet2':
        'கூடுதல் பார்வைகள் உருவாக்க பல கணக்குகள், போட்கள், ஸ்கிரிப்ட்கள், VPN மாற்றம், அல்லது எமுலேட்டர் குழுக்களை ஒரே நபர் தவறாக பயன்படுத்தக்கூடாது.',
    'termsUsingBullet3':
        'பரிசளிக்கும் விளம்பரங்கள், Firebase அங்கீகாரம், மற்றும் கட்டண மதிப்பாய்வு ஏற்கனவே உள்ள தள அமைப்பால் பாதுகாக்கப்படுகின்றன.',
    'termsViewsBullet1':
        'பயன்பாட்டில் காட்டப்படும் பார்வைகள் VideoMoney-க்குள் பயன்படுத்தப்படும் விளம்பர பரிசு அலகுகள் ஆகும்.',
    'termsViewsBullet2':
        'மதிப்பிடப்பட்ட வருமானம் தகவலுக்காக மட்டுமே; தள செயல்திறன், கொள்கை, மோசடி சரிபார்ப்பு, மற்றும் கட்டண மதிப்பாய்வின் அடிப்படையில் மாறக்கூடும்.',
    'termsViewsBullet3':
        'தினசரி போனஸ் பரிசுகள் தகுதியான செயல்பாட்டிற்கு மட்டுமே வரம்புபடுத்தப்பட்டவை; தவறான பயன்பாடு கண்டறியப்பட்டால் அவை நீக்கப்படலாம்.',
    'termsPayoutsBullet1':
        'குறைந்தபட்ச கட்டணம் 10,000 பார்வைகளாகவே இருக்கும்.',
    'termsPayoutsBullet2':
        'அனைத்து கட்டண கோரிக்கைகளுக்கும் கைமுறை நிர்வாகி ஒப்புதல் அவசியம்; அவை ஒப்புதல், நிராகரம், அல்லது செலுத்தப்பட்டது என குறிக்கப்படலாம்.',
    'termsPayoutsBullet3':
        'நிர்வாகி பணிச்சூழல் அனுமதித்தால் நிராகரிக்கப்பட்ட கட்டண கோரிக்கைகள் பயனர் இருப்பிற்கு திருப்பிச் சேர்க்கப்படலாம்.',
    'termsSupportBullet1':
        'உதவி மற்றும் ஆதரவு, பிழை அறிக்கைகள், நிர்வாகி பதில்கள், மற்றும் புஷ் அறிவிப்புகள் உங்கள் பயன்பாட்டு இன்பாக்ஸில் சேமிக்கப்படலாம்.',
    'termsSupportBullet2':
        'அறிவிப்புகளை இயக்குவதன் மூலம், பயன்பாட்டு புதுப்பிப்புகள், ஆதரவு பதில்கள், மற்றும் தினசரி நினைவூட்டல் செய்திகளை உங்கள் சாதனத்திற்கு அனுப்ப VideoMoney-க்கு நீங்கள் அனுமதி வழங்குகிறீர்கள்.',
    'termsSupportBullet3':
        'கடுமையான தவறான பயன்பாடு, தொல்லை, அல்லது மோசடி செயல்பாடு பயன்பாட்டின் அணுகலை கட்டுப்படுத்தக்கூடும்.',
    'settingsTitle': 'அமைப்புகள்',
    'statusPending': 'நிலுவையில்',
    'statusApproved': 'ஒப்புதல் பெற்றது',
    'statusPaid': 'செலுத்தப்பட்டது',
    'statusRejected': 'நிராகரிக்கப்பட்டது',
    'statusProcessing': 'செயலாக்கத்தில்',
    'statusFixed': 'சரிசெய்யப்பட்டது',
    'statusClosed': 'மூடப்பட்டது',
    'typeSupport': 'ஆதரவு',
    'typePayment': 'கட்டணம்',
    'typeBug': 'பிழை',
  },
  'te': {
    'appName': 'VideoMoney',
    'startingApp': 'VideoMoney ప్రారంభమవుతోంది',
    'preparingStartup':
        'Firebase, వాలెట్ డేటా మరియు రివార్డెడ్ ప్రకటనలు సిద్ధం అవుతున్నాయి...',
    'startupFailed': 'ప్రారంభం విఫలమైంది',
    'loadingWallet': 'మీ వాలెట్ లోడ్ అవుతోంది...',
    'unableRestoreSession':
        'ప్రస్తుతం మీ సెషన్‌ను తిరిగి పునరుద్ధరించలేకపోయాం.',
    'welcomeBack': 'మళ్లీ స్వాగతం',
    'createYourAccount': 'మీ ఖాతాను సృష్టించండి',
    'signInBody':
        'వ్యూలు సంపాదించడం మరియు చెల్లింపులను నిర్వహించడం కోసం సైన్ ఇన్ చేయండి.',
    'signUpBody':
        'రివార్డెడ్ వీడియోలు చూడటం ప్రారంభించి మీ బ్యాలెన్స్ పెంచుకోవడానికి నమోదు చేసుకోండి.',
    'login': 'లాగిన్',
    'signUp': 'సైన్ అప్',
    'email': 'ఇమెయిల్',
    'password': 'పాస్‌వర్డ్',
    'enterYourEmail': 'మీ ఇమెయిల్‌ను నమోదు చేయండి.',
    'enterValidEmail':
        'చెల్లుబాటు అయ్యే ఇమెయిల్ చిరునామాను నమోదు చేయండి.',
    'enterYourPassword':
        'మీ పాస్‌వర్డ్‌ను నమోదు చేయండి.',
    'useAtLeast6Chars': 'కనీసం 6 అక్షరాలు ఉపయోగించండి.',
    'rememberMe': 'నన్ను గుర్తుంచుకోండి',
    'forgotPassword': 'పాస్‌వర్డ్ మర్చిపోయారా?',
    'resetPassword': 'పాస్‌వర్డ్ రీసెట్ చేయండి',
    'emailHint': 'name@example.com',
    'cancel': 'రద్దు చేయండి',
    'sendResetLink': 'రీసెట్ లింక్ పంపండి',
    'passwordResetEmailSent':
        'పాస్‌వర్డ్ రీసెట్ ఇమెయిల్ {email} కు పంపబడింది.',
    'unableSendResetEmail':
        'పాస్‌వర్డ్ రీసెట్ ఇమెయిల్ పంపలేకపోయాం.',
    'firebaseAuthNotice':
        'Firebase Authentication యథావిధిగా ఉంటుంది. ఈ రీడిజైన్ ప్రదర్శన పొరను మాత్రమే నవీకరిస్తుంది.',
    'createAccount': 'ఖాతా సృష్టించండి',
    'secureAccessDashboard':
        'మీ ఆదాయ డ్యాష్‌బోర్డ్‌కు సురక్షిత ప్రాప్యత',
    'firestoreBalanceStored':
        'మీ చెల్లింపు చరిత్ర మరియు రివార్డ్ బ్యాలెన్స్ Firestore లో నిల్వ చేయబడతాయి',
    'authenticationFailed': 'ప్రామాణీకరణ విఫలమైంది.',
    'noUserSessionFound': 'వినియోగదారు సెషన్ కనబడలేదు.',
    'home': 'హోమ్',
    'earn': 'సంపాదించండి',
    'wallet': 'వాలెట్',
    'profile': 'ప్రొఫైల్',
    'rewardConfirmedViewsAdded':
        'రివార్డ్ నిర్ధారించబడింది. వ్యూలు జోడించబడ్డాయి.',
    'rewardedAdNotCompleted':
        'రివార్డెడ్ ప్రకటన పూర్తికాలేదు.',
    'watchVideo': 'వీడియో చూడండి',
    'loading': 'లోడ్ అవుతోంది...',
    'welcomeBackShort': 'మళ్లీ స్వాగతం,',
    'signedInUser': 'సైన్ ఇన్ చేసిన వినియోగదారు',
    'watchVideosEarnPaid':
        'వీడియోలు చూడండి, వ్యూలు సంపాదించండి, చెల్లింపు పొందండి.',
    'currentViews': 'ప్రస్తుత వ్యూలు',
    'videosWatched': 'చూసిన వీడియోలు',
    'progressToPayout': 'చెల్లింపుకు ప్రగతి',
    'youAreOnYourWay': 'మీరు సరైన దారిలో ఉన్నారు.',
    'earnViewsNow': 'ఇప్పుడే వ్యూలు సంపాదించండి.',
    'payoutUnlocked':
        'చెల్లింపు అన్‌లాక్ అయింది. మీరు వాలెట్‌లో చెల్లింపును అభ్యర్థించవచ్చు.',
    'moreViewsUntilPayout':
        'చెల్లింపుకు ఇంకా {views} వ్యూలు కావాలి.',
    'dailyBonus': 'రోజువారీ బోనస్',
    'watchDailyVideosBonus':
        'రోజుకు {videos} వీడియోలు చూస్తే బోనస్ వ్యూలు పొందండి.',
    'bonusClaimed': 'బోనస్ పొందారు',
    'bonus': 'బోనస్',
    'earnViewsTitle': 'వ్యూలు సంపాదించండి',
    'watchRewardedEarnViews':
        'రివార్డెడ్ వీడియోలు చూసి వెంటనే వ్యూలు సంపాదించండి.',
    'earnViews': 'వ్యూలు సంపాదించండి',
    'howItWorks': 'ఇది ఎలా పని చేస్తుంది',
    'watch': 'చూడండి',
    'watchShortVideo': 'ఒక చిన్న వీడియో చూడండి',
    'earnStep': 'సంపాదించండి',
    'getViewsReward': 'రివార్డ్‌గా వ్యూలు పొందండి',
    'cashOut': 'నగదీకరించండి',
    'reachViews': '{views} వ్యూలకు చేరుకోండి',
    'dailyChallenge': 'రోజువారీ ఛాలెంజ్',
    'watchTodayVideosBonus':
        'ఈరోజు {videos} వీడియోలు చూసి బోనస్ వ్యూలు పొందండి!',
    'totalVideosWatched': 'మొత్తం చూసిన వీడియోలు',
    'memberSince': 'సభ్యత్వం ప్రారంభం: {date}',
    'notAvailableYet': 'ఇంకా అందుబాటులో లేదు',
    'emailVerified': 'ఇమెయిల్ ధృవీకరించబడింది',
    'emailNotVerified': 'ఇమెయిల్ ధృవీకరించబడలేదు',
    'security': 'భద్రత',
    'firebaseProtected': 'Firebase రక్షణతో',
    'payoutReview': 'చెల్లింపు సమీక్ష',
    'adminApproval': 'అడ్మిన్ ఆమోదం',
    'readAdminReplies':
        'అడ్మిన్ సమాధానాలు మరియు నోటిఫికేషన్లు చదవండి',
    'notificationsPrivacy':
        'నోటిఫికేషన్లు, గోప్యత మరియు యాప్ సెట్టింగ్‌లు',
    'contactAdmin':
        'అడ్మిన్‌ను సంప్రదించి సందేశాలు పంపండి',
    'reportProblemBug':
        'సమస్య లేదా బగ్‌ను నివేదించండి',
    'rateApp': 'యాప్‌ను రేట్ చేయండి',
    'sendInAppRating':
        '1 నుండి 5 నక్షత్రాల వరకు మీ ఇన్-యాప్ రేటింగ్‌ను పంపండి',
    'aboutVideoMoney': 'VideoMoney గురించి',
    'reviewPayoutRequests':
        'చెల్లింపు అభ్యర్థనలను సమీక్షించండి',
    'logout': 'లాగ్ఔట్',
    'yourWallet': 'మీ వాలెట్',
    'availableViews': 'అందుబాటులో ఉన్న వ్యూలు',
    'estimatedPayout': 'అంచనా చెల్లింపు',
    'remainingToPayout': 'చెల్లింపుకు మిగిలినవి',
    'viewsUnit': 'వ్యూలు',
    'estimateOnly':
        'ఇది కేవలం అంచనా మాత్రమే. 50 వ్యూలు ≈ €0.01 మరియు అసలు ఆదాయం మారవచ్చు.',
    'requestPayout': 'చెల్లింపును అభ్యర్థించండి',
    'minPayout': 'కనీస చెల్లింపు',
    'processingTime': 'ప్రాసెసింగ్ సమయం',
    'approval': 'ఆమోదం',
    'adminReview': 'అడ్మిన్ సమీక్ష',
    'payoutMethods': 'చెల్లింపు పద్ధతులు',
    'payoutHistory': 'చెల్లింపు చరిత్ర',
    'noPayoutRequestsYet': 'ఇంకా చెల్లింపు అభ్యర్థనలు లేవు.',
    'pendingTimestamp': 'సమయం పెండింగ్‌లో ఉంది',
    'paypalSubtitle':
        'EUR, GBP లేదా USD లో చెల్లింపును అభ్యర్థించండి',
    'revolutSubtitle':
        'ఎంచుకున్న కరెన్సీతో వేగవంతమైన వాలెట్ చెల్లింపు',
    'bankTransferTitle': 'బ్యాంక్ బదిలీ',
    'bankTransferSubtitle':
        'మాన్యువల్ చెల్లింపుకు IBAN లేదా బ్యాంక్ ఖాతా సంఖ్యను జోడించండి',
    'viewFullHistory': 'పూర్తి చరిత్రను చూడండి',
    'inbox': 'ఇన్‌బాక్స్',
    'markAllRead': 'అన్నీ చదివినట్లుగా గుర్తించండి',
    'noInboxMessagesYet': 'ఇంకా ఇన్‌బాక్స్ సందేశాలు లేవు.',
    'newBadge': 'కొత్తది',
    'rateAppQuestion': 'మీరు VideoMoney ను ఎలా రేట్ చేస్తారు?',
    'choose1to5':
        '1 నుండి 5 నక్షత్రాల మధ్య ఎంచుకోండి. తర్వాత మీ రేటింగ్‌ను నవీకరించవచ్చు.',
    'saveRating': 'రేటింగ్‌ను సేవ్ చేయండి',
    'thanksForRating': 'మీ రేటింగ్‌కు ధన్యవాదాలు.',
    'requestPayoutTitle': 'చెల్లింపును అభ్యర్థించండి',
    'payoutRules': 'చెల్లింపు నియమాలు',
    'minimumPayoutIs': 'కనీస చెల్లింపు {views} వ్యూలు.',
    'processingCanTake':
        'అడ్మిన్ ఆమోదం తర్వాత ప్రాసెసింగ్‌కు గరిష్టంగా {days} రోజులు పట్టవచ్చు.',
    'everyRequestReviewed':
        'ప్రతి అభ్యర్థన చెల్లింపుకు ముందు అడ్మిన్ ద్వారా సమీక్షించబడుతుంది.',
    'useBankAddIban':
        'మాన్యువల్ బదిలీ కోసం Bank ను ఉపయోగించి మీ IBAN లేదా బ్యాంక్ ఖాతా సంఖ్యను జోడించండి.',
    'submitUsingBalance':
        'మీ వ్యూ బ్యాలెన్స్ ఉపయోగించి చెల్లింపు అభ్యర్థనను పంపండి.',
    'estimatedEarningsNotGuaranteed':
        'అంచనా ఆదాయమే. 50 పూర్తయిన వ్యూలు ≈ €0.01 మరియు ఇది హామీ ఇచ్చిన చెల్లింపు కాదు.',
    'payoutCurrency': 'చెల్లింపు కరెన్సీ',
    'payoutMethod': 'చెల్లింపు పద్ధతి',
    'viewsToRequest': 'అభ్యర్థించాల్సిన వ్యూలు',
    'minimumViewsHelper': 'కనీసం 10,000 వ్యూలు',
    'enterAmount': 'ఒక మొత్తాన్ని నమోదు చేయండి.',
    'enterValidPositiveNumber':
        'చెల్లుబాటు అయ్యే ధన సంఖ్యను నమోదు చేయండి.',
    'accountHolderName': 'ఖాతాదారుడి పేరు',
    'enterAccountHolderName':
        'ఖాతాదారుడి పేరును నమోదు చేయండి.',
    'paypalEmail': 'PayPal ఇమెయిల్',
    'enterPaypalEmail': 'PayPal ఇమెయిల్‌ను నమోదు చేయండి.',
    'enterValidPaypalEmail':
        'చెల్లుబాటు అయ్యే PayPal ఇమెయిల్‌ను నమోదు చేయండి.',
    'revolutUsername': 'Revolut వినియోగదారు పేరు',
    'revolutExample': 'ఉదాహరణ: @yourname',
    'enterRevolutUsername':
        'మీ Revolut వినియోగదారు పేరును నమోదు చేయండి.',
    'bankName': 'బ్యాంక్ పేరు',
    'enterBankName': 'మీ బ్యాంక్ పేరును నమోదు చేయండి.',
    'iban': 'IBAN',
    'ibanOptional':
        'మీరు బ్యాంక్ ఖాతా సంఖ్య ఇస్తే ఇది ఐచ్ఛికం',
    'bankAccountNumber': 'బ్యాంక్ ఖాతా సంఖ్య',
    'bankRequiredIfNoIban':
        'మీరు IBAN ఇవ్వకపోతే ఇది తప్పనిసరి',
    'enterIbanOrBank':
        'IBAN లేదా బ్యాంక్ ఖాతా సంఖ్యను నమోదు చేయండి.',
    'submitRequest': 'అభ్యర్థనను సమర్పించండి',
    'payoutRequestSubmitted':
        'చెల్లింపు అభ్యర్థన సమర్పించబడింది.',
    'payoutHistoryTitle': 'చెల్లింపు చరిత్ర',
    'unableLoadPayoutHistory':
        'ప్రస్తుతం చెల్లింపు చరిత్రను లోడ్ చేయలేకపోయాం.',
    'currencyLabel': 'కరెన్సీ: {currency}',
    'accountHolderLabel': 'ఖాతాదారు: {value}',
    'notProvided': 'ఇవ్వలేదు',
    'helpSupport': 'సహాయం & మద్దతు',
    'openSupportTicket': 'సపోర్ట్ టికెట్ తెరవండి',
    'describeIssueAdminReply':
        'మీ సమస్యను వివరించండి, అడ్మిన్ మీ ఇన్‌బాక్స్‌లో సమాధానం ఇవ్వగలరు.',
    'subject': 'విషయం',
    'helpSubjectHint': 'మీకు ఏ విషయంలో సహాయం కావాలి?',
    'message': 'సందేశం',
    'messageHint': 'మీ సందేశాన్ని ఇక్కడ రాయండి...',
    'send': 'పంపండి',
    'sending': 'పంపుతోంది...',
    'openInbox': 'ఇన్‌బాక్స్ తెరవండి',
    'yourTickets': 'మీ టికెట్లు',
    'noSupportTicketsYet': 'ఇంకా సపోర్ట్ టికెట్లు లేవు.',
    'supportMessageSent': 'సపోర్ట్ సందేశం పంపబడింది.',
    'adminReply': 'అడ్మిన్ సమాధానం: {value}',
    'reportBug': 'బగ్‌ను నివేదించండి',
    'tellUsWhatHappened': 'ఏం జరిగిందో మాకు చెప్పండి',
    'includeStepsExpected':
        'మళ్లీ ఎలా సంభవిస్తుందో దశలు మరియు మీరు ఏమి చూడాలని ఆశించారో చేర్చండి.',
    'title': 'శీర్షిక',
    'shortSummary': 'చిన్న సారాంశం',
    'description': 'వివరణ',
    'describeBug': 'బగ్‌ను వివరించండి...',
    'submit': 'సమర్పించండి',
    'bugReportSubmitted': 'బగ్ నివేదిక సమర్పించబడింది.',
    'aboutTagline':
        'ప్రీమియం డార్క్ థీమ్ • నియాన్ గ్రీన్ UI',
    'version': 'వెర్షన్',
    'minimumPayoutLabel': 'కనీస చెల్లింపు',
    'processingTimeLabel': 'ప్రాసెసింగ్ సమయం',
    'reviewLabel': 'సమీక్ష',
    'estimatedEarningsOnlyPolicies':
        'అంచనా ఆదాయమే. ప్రకటనల పనితీరు మరియు విధాన నియమాల ఆధారంగా అసలు ఆదాయం మారవచ్చు.',
    'privacyPolicy': 'గోప్యతా విధానం',
    'termsOfService': 'సేవా నిబంధనలు',
    'notifications': 'నోటిఫికేషన్లు',
    'enableNotifications': 'నోటిఫికేషన్లను ప్రారంభించండి',
    'generalAppNotifications': 'సాధారణ యాప్ నోటిఫికేషన్లు',
    'dailyReminder': 'రోజువారీ గుర్తుచేయింపు',
    'dailyReminderSubtitle':
        'రోజువారీ బోనస్ పూర్తి చేయడానికి గుర్తుచేయింపును పొందండి',
    'settingsSaved': 'సెట్టింగ్‌లు సేవ్ అయ్యాయి.',
    'privacy': 'గోప్యత',
    'appVersion': 'యాప్ వెర్షన్',
    'save': 'సేవ్ చేయండి',
    'saving': 'సేవ్ అవుతోంది...',
    'termsUsingTitle': 'VideoMoney వినియోగం',
    'termsViewsTitle': 'వ్యూలు మరియు బహుమతులు',
    'termsPayoutsTitle': 'చెల్లింపులు మరియు సమీక్ష',
    'termsSupportTitle': 'సహాయం మరియు సందేశాలు',
    'termsUsingBullet1':
        'మీరు ఖచ్చితమైన ఖాతా సమాచారాన్ని ఉపయోగించాలి.',
    'termsUsingBullet2':
        'ఒక వ్యక్తి అదనపు వ్యూలు సృష్టించడానికి బహుళ ఖాతాలు, బాట్లు, స్క్రిప్టులు, VPN రొటేషన్ లేదా ఎమ్యులేటర్ ఫార్మ్‌లను దుర్వినియోగం చేయకూడదు.',
    'termsUsingBullet3':
        'రివార్డెడ్ ప్రకటనలు, Firebase ప్రామాణీకరణ మరియు చెల్లింపు సమీక్ష ప్రస్తుత ప్లాట్‌ఫారమ్ సెటప్ ద్వారా రక్షించబడుతూనే ఉంటాయి.',
    'termsViewsBullet1':
        'యాప్‌లో చూపించే వ్యూలు VideoMoney లో ఉపయోగించే ప్రమోషనల్ రివార్డ్ యూనిట్‌లు.',
    'termsViewsBullet2':
        'అంచనా ఆదాయం కేవలం సమాచార కోసం మాత్రమే మరియు ప్లాట్‌ఫారమ్ పనితీరు, విధానం, మోసం తనిఖీలు మరియు చెల్లింపు సమీక్ష ఆధారంగా మారవచ్చు.',
    'termsViewsBullet3':
        'రోజువారీ బోనస్ బహుమతులు అర్హమైన కార్యకలాపాలకు మాత్రమే పరిమితం; దుర్వినియోగం గుర్తిస్తే తొలగించబడవచ్చు.',
    'termsPayoutsBullet1': 'కనీస చెల్లింపు 10,000 వ్యూలుగానే ఉంటుంది.',
    'termsPayoutsBullet2':
        'అన్ని చెల్లింపు అభ్యర్థనలకు మాన్యువల్ అడ్మిన్ ఆమోదం అవసరం మరియు అవి ఆమోదించబడవచ్చు, తిరస్కరించబడవచ్చు లేదా చెల్లించబడినట్లుగా గుర్తించబడవచ్చు.',
    'termsPayoutsBullet3':
        'అడ్మిన్ వర్క్‌ఫ్లో అనుమతిస్తే తిరస్కరించబడిన చెల్లింపు అభ్యర్థనలు వినియోగదారు బ్యాలెన్స్‌కు తిరిగి జమ చేయబడవచ్చు.',
    'termsSupportBullet1':
        'సహాయం & మద్దతు, బగ్ నివేదికలు, అడ్మిన్ సమాధానాలు మరియు పుష్ నోటిఫికేషన్లు మీ ఇన్-యాప్ ఇన్‌బాక్స్‌లో నిల్వ చేయబడవచ్చు.',
    'termsSupportBullet2':
        'నోటిఫికేషన్లను ప్రారంభించడం ద్వారా, VideoMoney మీ పరికరానికి యాప్ నవీకరణలు, సపోర్ట్ సమాధానాలు మరియు రోజువారీ గుర్తుచేయింపు సందేశాలను పంపడానికి మీరు అనుమతిస్తారు.',
    'termsSupportBullet3':
        'తీవ్రమైన దుర్వినియోగం, వేధింపు లేదా మోసపూరిత కార్యకలాపాలు యాప్ ప్రాప్యతను పరిమితం చేయించవచ్చు.',
    'settingsTitle': 'సెట్టింగ్‌లు',
    'statusPending': 'పెండింగ్',
    'statusApproved': 'ఆమోదించబడింది',
    'statusPaid': 'చెల్లించబడింది',
    'statusRejected': 'తిరస్కరించబడింది',
    'statusProcessing': 'ప్రాసెసింగ్‌లో ఉంది',
    'statusFixed': 'పరిష్కరించబడింది',
    'statusClosed': 'మూసివేయబడింది',
    'typeSupport': 'సపోర్ట్',
    'typePayment': 'చెల్లింపు',
    'typeBug': 'బగ్',
  },
};
